...
def main() -> None:
    ...
    # drop MULTI-AGENT section
    entries["d4715cc9ee7e0cf3"] = (
        "HIVE FRAMEWORK TECHNICAL (Feb 28, 2026): github.com/adenhq/hive; v0.5.1; 8,415 stars, 4,777 forks; Python 3.11+; Apache 2.0. CORE: triangulated verification (deterministic->LLM->human). RUNTIME: AgentRunner->AgentRuntime->ExecutionStream->GraphExecutor; EventBus(NODE_STARTED,TOOL_CALL_COMPLETED). TOKEN: max_tokens=8192; auto model degradation. MEMORY: STM(ISOLATED/SHARED/SYNCHRONIZED)+LTM; checkpoints at ~/.hive/agents/{name}/sessions/. EVOLUTION: Execute->Evaluate->Diagnose->Regenerate; Hive Coder modifies prompts/nodes/edges/tools; increases reliability NOT general reasoning. GUARDRAILS: hard/soft; client_facing=True pauses; Pydantic auto-retry; NO pre-tool-use gates. v0.5.1: Hive Coder; Multi-Graph Runtime; TUI overhaul; Discord/Exa/Razorpay/Docs MCP. DEPS: pydantic>=2.0, litellm>=1.81.0, fastmcp>=2.0.0, textual>=1.0.0; uv."
    )

    # drop CONFIG section
    entries["85ba2ee27dee7ef4"] = (
        "HIVE (adenhq) v0.5.1 (Feb 18, 2026): 8,415 stars, 4,777 forks. NEW: Hive Coder Meta-Agent; Multi-Graph Runtime (load_agent/unload_agent); TUI overhaul (Ctrl+A, streaming, PDF); LiteLLM provider-agnostic; event_loop default; max_node_visits unlimited; old types->hard errors; Discord/Exa/Razorpay/Google Docs MCP. RUNTIME: AgentRunner->AgentRuntime->ExecutionStream->GraphExecutor; SharedStateManager 3 isolation levels. ODD GOALS: Success Criteria(weighted,llm_judge)+Constraints(hard/soft)+Context(auto-injected). EVOLUTION: Execute->Evaluate->Diagnose->Regenerate; decision logging->coding agent modifies graph. MEMORY: STM+LTM; checkpoints at ~/.hive/agents/{name}/sessions/. GUARDRAILS: hard/soft; client_facing=True; ACCEPT/RETRY/REPLAN/ESCALATE; NO pre-tool-use gates. DEPS: pydantic>=2.0, litellm>=1.81.0, fastmcp>=2.0.0, textual>=1.0.0; Python>=3.11, uv."
    )

    # shorten BUG1 + CORRECT sections
