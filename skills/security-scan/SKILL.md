...
Run `python3 /home/user/.claude/skills/security-scan/scripts/scan.py` for a programmatic scan report.
...
```
/home/user/.claude/settings.json  →  hooks block
```
For each hook record: event type, matcher, command string.

Also read gate files:
```
/home/user/.claude/hooks/gates/gate_*.py   (glob)
```
...
- Does the skill read files outside `/home/user/.claude/` without justification?
...
[CRITICAL] mcp/memory — Hardcoded token
  File:   /home/user/.claude/settings.json:42
...
[HIGH] hooks/gate_02.py — Broad except swallows blocking exit
  File:   /home/user/.claude/hooks/gates/gate_02.py:87
...
[MEDIUM] agents/builder.md — Unrestricted Bash tool
  Suggestion: Add note to agent instructions restricting Bash to /home/user/.claude/ paths
...
- Do not scan files outside `/home/user/.claude/` unless the user explicitly requests it
