...
**Memory system:**
```bash
cat /home/user/.claude/stats-cache.json
```
...
for _ in range(N):
    ...
    subprocess.run(
        ['python3', '/home/user/.claude/hooks/enforcer.py', '--event', 'PreToolUse'],
...
```bash
python3 /home/user/.claude/hooks/benchmarks/benchmark_io.py 2>&1 | grep -E "(Mean|p50|p95|p99|Speedup|Total|SUMMARY)" | head -20
```
