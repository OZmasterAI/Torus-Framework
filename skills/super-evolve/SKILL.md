...
```bash
# Gates
ls /home/user/.claude/hooks/gates/

# Hook scripts (registered in settings.json)
python3 -c "import json; d=json.load(open('/home/user/.claude/settings.json')); [print(h) for h in d.get('hooks', {})]"

# Skills
ls /home/user/.claude/skills/

# MCP tools (registered servers)
python3 -c "import json; d=json.load(open('/home/user/.claude/settings.json')); [print(s) for s in d.get('mcpServers', {}).keys()]"

# Scripts
ls /home/user/.claude/scripts/ 2>/dev/null || echo "(no scripts dir)"

# Plugins
ls /home/user/.claude/plugins/ 2>/dev/null || echo "(no plugins dir)"
```
...
stat -c "%n %y" /home/user/.claude/hooks/gates/gate_*.py | sort -k2
```

### 2b. Test coverage check
```bash
# Count test cases per gate in test_framework.py
grep -c "gate_0[0-9]" /home/user/.claude/hooks/test_framework.py 2>/dev/null || \
grep -c "gate_1[0-9]" /home/user/.claude/hooks/test_framework.py 2>/dev/null || \
...
### 2d. Skills audit
For each skill in `/home/user/.claude/skills/`:
...
```bash
python3 /home/user/.claude/hooks/test_framework.py 2>&1 | tail -5
```
...
```bash
python3 /home/user/.claude/hooks/test_framework.py 2>&1 | tail -10
```
...
```bash
ls /home/user/.claude/ARCHITECTURE.md 2>/dev/null
```
...
```bash
python3 /home/user/.claude/hooks/test_framework.py 2>&1
```
