...
- **Repositioned CMP icon** in `hooks/statusline.py` — moved 📦CMP:N from being appended to context % string to its own pipe-delimited element before context %. Now reads: `HP:bar | 📦CMP:N | 62%` instead of `HP:bar | 62% 📦CMP:N`.
...
5. Memory compaction planning — revisit at 500+ (currently 370)
...
## Service Status
- Memory MCP: 370 memories
- Tests: 981 passed, 0 failed
- Framework version: v2.4.2
- Ramdisk: active at /run/user/1000/claude-hooks
- Statusline: 2-line layout, CMP before context %, 5-tier context colors, model-based bracket colors
