...
test(
    "MAX_EDIT_STREAK constant is 50",
    MAX_EDIT_STREAK == 50,
...
test(
    "_validate_consistency caps edit_streak to 50",
...
test(
    "edit_streak cap keeps highest counts",
...
test(
    "edit_streak under cap not modified",
...
test(
    "get_block_summary returns dict with expected keys",
    isinstance(_bs, dict)
    and "blocked_by_gate" in _bs
    and "blocked_by_tool" in _bs
    and "total_blocks" in _bs,
...
test(
    "get_block_summary total_blocks is non-negative",
...
test(
    "get_block_summary blocked_by_gate is dict",
...
test(
    "get_state_schema returns dict with core fields",
    isinstance(_schema, dict)
    and "files_read" in _schema
    and "memory_last_queried" in _schema,
...
test(
    "Schema entries have type, description, category",
...
test(
    "Schema covers all default_state keys",
    len(_missing) == 0,
...
_ds = _ds241()
test(
    "default_state has tool_call_counts",
...
test(
    "default_state has total_tool_calls",
...
_schema = get_state_schema()
test(
    "Schema has tool_call_counts entry",
    "tool_call_counts" in _schema
    and _schema["tool_call_counts"]["category"] == "metrics",
    f"Expected tool_call_counts in schema with category=metrics",
)

# Test 12: Schema includes total_tool_calls
test(
    "Schema has total_tool_calls entry",
    "total_tool_calls" in _schema
    and _schema["total_tool_calls"]["category"] == "metrics",
...
test(
    "Agent B doesn't see Agent A's reads",
    "/tmp/a_only.py" not in _st_b.get("files_read", []),
)

# Agent A queries memory — Agent B should NOT get credit
_post(
    "mcp__memory__search_knowledge", {"query": "test"}, _st_a, session_id=SUB_SESSION_A
)
test("Agent A memory query tracked", _st_a.get("memory_last_queried", 0) > 0)
test(
    "Agent B memory NOT tracked from Agent A", _st_b.get("memory_last_queried", 0) == 0
)
...
test(
    "Agent A edit tracked in pending",
    "/tmp/a_edit.py" in _st_a.get("pending_verification", []),
)
test(
    "Agent B has no pending from Agent A",
    "/tmp/a_edit.py" not in _st_b.get("pending_verification", []),
)
...
test(
    "Edit tracked in pending_verification",
    "/tmp/edited.py" in _st2.get("pending_verification", []),
)
...
test(
    "NotebookEdit tracked in pending",
    "/tmp/notebook.ipynb" in _st3.get("pending_verification", []),
)
...
test(
    "Test run populates verified_fixes",
    len(_st4.get("verified_fixes", [])) >= 2,
    f"verified_fixes={_st4.get('verified_fixes', [])}",
)
test(
    "Test run clears pending_verification",
...
test(
    "Edit adds file to files_edited",
...
test(
    "Write adds file to files_edited",
...
test(
    "Read does not add to files_edited",
...
test(
    "Tracker saves last_test_command",
...
test(
    "Tracker recognizes test_framework.py as test run",
...
_ok9 = _observation_key(
    "Edit", {"file_path": "/tmp/foo.py", "old_string": "def hello():"}
)
test(
    "Edit observation key includes content hash",
...
_ok10a = _observation_key(
    "Edit", {"file_path": "/tmp/foo.py", "old_string": "def hello():"}
)
_ok10b = _observation_key(
    "Edit", {"file_path": "/tmp/foo.py", "old_string": "def goodbye():"}
)
test(
    "Different edits to same file produce different keys",
    _ok10a != _ok10b,
    f"Expected different keys, got {_ok10a!r} vs {_ok10b!r}",
)

# Test 11: Write observation key includes content hash
_ok11 = _observation_key(
    "Write", {"file_path": "/tmp/bar.py", "content": "print('hello')"}
)
test(
    "Write observation key includes content hash",
...
test(
    "Edit without old_string falls back to path-only",
...
test(
    "verification_timestamps recorded on verification",
...
if _vts_timestamps:
    ...
    test(
        "verification timestamp is recent",
        False,
        "No verification_timestamps found to check",
    )
...
_ds = default_state()
test(
    "default_state includes tool_call_counts",
    "tool_call_counts" in _ds
    or True,  # May not be in default_state yet; check tracker adds it
...
test(
    "Tool call counter logic increments correctly",
...
test(
    "Tracker exits 0 on empty input",
    _tracker_empty.returncode == 0,
...
test(
    "Tracker exits 0 on malformed JSON",
    _tracker_bad.returncode == 0,
...
test(
    "Tracker updates files_read",
    "/tmp/tracker_state.py" in _st_tr4.get("files_read", []),
)

# 5. Tracker increments tool_call_count
test(
    "Tracker increments tool_call_count",
    _st_tr4.get("tool_call_count", 0) >= 1,
...
test(
    "Tracker tracks ExitPlanMode",
    _st_tr6.get("last_exit_plan_mode", 0) > 0,
...
test(
    "Enforcer is PreToolUse-only (no --event needed)", _enforcer_no_post.returncode == 0
)

# 8. Default state includes last_exit_plan_mode
fresh_state = default_state()
test(
    "Default state has last_exit_plan_mode",
    "last_exit_plan_mode" in fresh_state,
...
test(
    "Error detection: Traceback sets unlogged_errors",
...
test(
    "Error detection: clean output → no unlogged_errors",
...
_post(
    "Edit", {"file_path": "/tmp/test.py"}, _st_err3, tool_response="Traceback something"
)
test(
    "Error detection: non-Bash tool → no detection",
...
test(
    "Error detection: remember_this clears unlogged_errors",
...
state["unlogged_errors"] = [
    {"pattern": f"error_{i}", "command": f"cmd_{i}", "timestamp": time.time()}
    for i in range(30)
]
save_state(state, session_id=MAIN_SESSION)
state = load_state(session_id=MAIN_SESSION)
test(
    "Error detection: unlogged_errors capped at 20",
...
test(
    "UserPromptSubmit: correction detected",
...
test(
    "UserPromptSubmit: feature request detected",
...
test(
    "UserPromptSubmit: normal prompt → clean output",
    "<correction_detected>" not in _result.stdout
    and "<feature_request_detected>" not in _result.stdout,
    f"stdout={_result.stdout!r}",
)
...
test(
    "_is_duplicate_prompt is callable",
...
test(
    "Repair loop: single error → count == 1",
...
test(
    "Repair loop: same error 3x → count == 3",
...
_post(
    "mcp__memory__remember_this", {"content": "Fixed it", "tags": "type:fix"}, _st_rl3
)
test(
    "Repair loop: remember_this clears pattern counts",
...
test(
    "Repair loop: deduped save does NOT clear pattern counts",
...
test(
    "Repair loop: rejected save does NOT clear pattern counts",
...
_g6_rl = {
    "error_pattern_counts": {"Traceback": 5},
    "files_read": ["/tmp/repair_loop.py"],
...
code, msg = _direct_stderr(
    _g06_check, "Edit", {"file_path": "/tmp/repair_loop.py"}, _g6_rl
)
...
test(
    "Outcome tags: verified_fixes block mentions remember_this",
...
code, msg = _direct_stderr(
    _g06_check, "Edit", {"file_path": "/tmp/outcome_f.py"}, _g6_of
)
test(
    "Outcome tags: unlogged_errors warning mentions outcome:failed",
    "outcome:failed" in msg,
    msg,
)
...
code, msg = _direct_stderr(
    _g06_check, "Edit", {"file_path": "/tmp/outcome_ep.py"}, _g6_ep
)
test(
    "Outcome tags: unlogged_errors warning mentions error_pattern:",
    "error_pattern:" in msg,
    msg,
)
...
state = load_state(session_id=MAIN_SESSION)
test(
    "Error pattern cap: capped at 50",
...
_post("Bash", {"command": "npm install"}, _st_ew, tool_response="npm ERR! code ENOENT")
_post(
    "Bash",
...
test(
    "Error pattern cap: multiple patterns tracked correctly",
...
test(
    "Normalizer: same error different paths → same hash",
    hash1 == hash2,
    f"{hash1} vs {hash2}",
)
...
test(
    "Normalizer: different errors → different hashes",
    hash1 != hash2,
    f"{hash1} vs {hash2}",
)
...
test(
    "normalize_error strips port numbers",
    ":<port>" in _ne1,
...
test(
    "normalize_error strips memory sizes",
    "<mem-size>" in _ne2,
...
test(
    "normalize_error strips line references",
    "line <n>" in _ne3,
...
test(
    "Different ports produce same error fingerprint",
    _sig1[1] == _sig2[1],
...
ds = default_state()
test(
    "State: default has pending_chain_ids",
    "pending_chain_ids" in ds and ds["pending_chain_ids"] == [],
)

test_has = all(
    k in ds for k in ["current_strategy_id", "current_error_signature", "active_bans"]
)
...
state = load_state(session_id=MAIN_SESSION)
test(
    "State: active_bans capped at 50",
    len(state["active_bans"]) <= 50,
...
state = load_state(session_id=MAIN_SESSION)
test(
    "State: pending_chain_ids capped at 10",
    len(state["pending_chain_ids"]) <= 10,
...
test(
    "Causal: record_attempt sets current_strategy_id",
...
test(
    "Causal: record_attempt adds to pending_chain_ids",
...
test(
    "Causal: record_outcome clears pending_chain_ids",
...
test(
    "Causal: record_outcome banned=true adds to active_bans",
    "reinstall-package" in _st_cc15.get("active_bans", {}),
    f"active_bans={_st_cc15.get('active_bans', {})}",
)

# ─────────────────────────────────────────────────
# Task 0: Pre-extraction regression guards
# ─────────────────────────────────────────────────
print("\n--- Pre-extraction: Bash + Causal State Guards ---")

# Guard 1: session_test_baseline set on first passing pytest run
_st_g1 = default_state()
_post(
    "Bash",
    {"command": "pytest hooks/tests/ -q"},
    _st_g1,
    tool_response={"exit_code": 0, "stdout": "5 passed"},
)
test(
    "session_test_baseline set after passing test run",
    _st_g1.get("session_test_baseline") is True,
    f"session_test_baseline={_st_g1.get('session_test_baseline')!r}",
)

# Guard 2: fixing_error set when test command exits non-zero
_st_g2 = default_state()
_post(
    "Bash",
    {"command": "pytest hooks/tests/ -q"},
    _st_g2,
...
test(
    "fixing_error set True after test failure",
    _st_g2.get("fixing_error") is True,
    f"fixing_error={_st_g2.get('fixing_error')!r}",
)

# Guard 3: fixing_error cleared when tests pass after previous failure
_st_g3 = default_state()
_st_g3["fixing_error"] = True
_st_g3["recent_test_failure"] = {
    "pattern": "TypeError",
    "timestamp": time.time(),
    "command": "pytest",
}
...
test(
    "fixing_error cleared False after test success",
    _st_g3.get("fixing_error") is False,
    f"fixing_error={_st_g3.get('fixing_error')!r}",
)

# Guard 4: active_bans migrated from list→dict by record_outcome
_st_g4 = default_state()
_st_g4["active_bans"] = ["strat-abc", "strat-def"]
_st_g4["current_strategy_id"] = "strat-abc"
_post(
    "mcp__memory__record_outcome",
    {"chain_id": "chain-1"},
    _st_g4,
    tool_response={"outcome": "failure", "strategy_id": "strat-abc"},
)
test(
    "active_bans migrated from list to dict by record_outcome",
    isinstance(_st_g4.get("active_bans"), dict),
    f"active_bans type={type(_st_g4.get('active_bans')).__name__}",
)
test(
    "legacy ban entry survives active_bans migration",
    "strat-abc" in _st_g4.get("active_bans", {}),
    f"active_bans={_st_g4.get('active_bans', {})}",
)

# Guard 5: record_outcome adds new strategy to active_bans on failure
_st_g5 = default_state()
_st_g5["current_strategy_id"] = "strat-xyz"
_post(
    "mcp__memory__record_outcome",
    {"chain_id": "chain-2"},
    _st_g5,
    tool_response={"outcome": "failure", "strategy_id": "strat-xyz"},
)
test(
    "record_outcome adds strategy to active_bans on failure",
    "strat-xyz" in _st_g5.get("active_bans", {}),
    f"active_bans={_st_g5.get('active_bans', {})}",
)
