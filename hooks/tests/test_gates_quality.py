...
code, msg = _direct(
    _g04_check(
        "Task",
        {"subagent_type": "researcher", "model": "sonnet", "description": "research"},
...
code, msg = _direct(
    _g04_check(
        "Task",
        {"subagent_type": "Explore", "model": "sonnet", "description": "explore"},
        {"memory_last_queried": 0},
    )
)
...
code, msg = _direct(
    _g04_check(
        "Task",
        {"subagent_type": "builder", "model": "sonnet", "description": "build"},
        {"memory_last_queried": 0},
    )
)
...
test(
    "Gate 4 tracks exemption for LIVE_STATE.json",
...
test(
    "Gate 4 exemption count increments",
...
test(
    "Gate 4 EXEMPT_BASENAMES includes LIVE_STATE.json and CLAUDE.md",
...
test(
    "Gate 5: block message mentions GATE 5",
    "GATE 5" in (_g5_result_3.message or ""),
    _g5_result_3.message,
)

# Re-editing file_a.py should be ALLOWED (same-file exemption)
code, msg = _direct(
    _g05_check("Edit", {"file_path": "/tmp/file_a.py"}, _g5_state_3pending)
)
...
code, msg = _direct(
    _g05_check("Edit", {"file_path": "/tmp/file_d.py"}, _g5_state_cleared)
)
...
test(
    "is_test_file detects test_ prefix",
...
test(
    "is_test_file detects _test suffix",
...
test(
    "is_test_file rejects non-test files",
...
test(
    "Gate 5 allows test file edit with pending verifications",
...
test(
    "Gate 5 blocks at 5 unverified files",
...
test(
    "Gate 5 blocks at 4 unverified files (above threshold 3)",
...
test(
    "Gate 6: block message mentions GATE 6",
    "GATE 6" in (_g6_block_result.message or ""),
    _g6_block_result.message,
)
...
rc12_5, stderr12_5 = _direct_stderr(
    _g06_check,
    "Edit",
...
test(
    "Gate 6 plan mode warning mentions plan mode",
...
_g6pm6 = {
    "files_read": ["foo.py"],
    "memory_last_queried": time.time(),
    "last_exit_plan_mode": time.time() - 60,
    "verified_fixes": [],
    "unlogged_errors": [],
    "pending_chain_ids": [],
    "gate6_warn_count": 0,
}
rc12_6, stderr12_6 = _direct_stderr(
    _g06_check,
    "Edit",
    {"file_path": "foo.py", "old_string": "a", "new_string": "b"},
    _g6pm6,
...
test(
    "Gate 6 plan mode — no warning when memory is fresh",
...
rc12_7, stderr12_7 = _direct_stderr(
    _g06_check,
    "Edit",
    {"file_path": "foo.py", "old_string": "a", "new_string": "b"},
    _g6pm7,
...
test(
    "Gate 6 plan mode — warns when plan exited without memory save",
...
_g6pm8 = {
    "files_read": ["foo.py"],
    "memory_last_queried": time.time() - 3600,
    "last_exit_plan_mode": time.time() - 2000,
    "verified_fixes": [],
    "unlogged_errors": [],
    "pending_chain_ids": [],
    "gate6_warn_count": 0,
}
rc12_8, stderr12_8 = _direct_stderr(
    _g06_check,
    "Edit",
    {"file_path": "foo.py", "old_string": "a", "new_string": "b"},
    _g6pm8,
...
test(
    "Gate 6 plan mode — stale plan auto-forgiven",
...
test(
    "Gate 6 warns with edit streak >= 3",
...
test(
    "Gate 6 no warning with edit streak < 3",
    _g6_result2.severity != "warn"
    or len(_g6_state2.get("verified_fixes", [])) >= BLOCK_THRESHOLD,
...
test(
    "Gate 6 edit streak shows basename",
...
test(
    "Gate 6 edit streak shows count",
...
_g6_ro_result1 = gate6_check(
    "Task", {"subagent_type": "researcher", "model": "sonnet"}, _g6_ro_state1
)
test(
    "Gate 6: Task researcher exempt (read-only)",
    not _g6_ro_result1.blocked,
    f"Expected not blocked, got blocked={_g6_ro_result1.blocked}",
)

_g6_ro_result2 = gate6_check(
    "Task", {"subagent_type": "Explore", "model": "sonnet"}, _g6_ro_state1
)
test(
    "Gate 6: Task Explore exempt (read-only)",
...
_g6_ro_result3 = gate6_check(
    "Task", {"subagent_type": "builder", "model": "sonnet"}, _g6_ro_state1
)
test(
    "Gate 6: Task builder NOT exempt (write agent)",
...
test(
    "edit streak risk 0 hotspots → safe",
...
test(
    "edit streak risk 1 hotspot → warning",
...
test(
    "edit streak risk 2 hotspots → warning",
...
test(
    "edit streak risk 3 hotspots → critical",
...
test(
    "Gate 6 warns about recent repair loop",
...
_g6d_state6["error_windows"] = [
    {
        "pattern": "ImportError",
        "first_seen": time.time() - 1800,
        "last_seen": time.time() - 700,
        "count": 5,
    }
]
...
sys.stderr = _orig_stderr229
test(
    "Gate 6 skips stale repair loop (>10min)",
...
sys.stderr = _orig_stderr229
test(
    "Gate 6 warns when pattern not in error_windows (defensive)",
...
sys.stderr = _orig_stderr229
test(
    "Gate 6 no repair loop for count < 3",
...
_g6d_state8b["error_windows"] = [
    {
        "pattern": "ToolFail:WebFetch",
...
sys.stderr = _orig_stderr229
test(
    "Gate 6 ToolFail:WebFetch does NOT warn on Edit (scoped)",
...
_g6d_state8c["error_windows"] = [
    {
        "pattern": "ToolFail:Bash",
        "first_seen": time.time() - 60,
        "last_seen": time.time() - 10,
        "count": 4,
    }
...
sys.stderr = _orig_stderr229
test(
    "Gate 6 ToolFail:Bash DOES warn on Bash (scoped match)",
...
sys.stderr = _orig_stderr229
test(
    "Gate 6 non-ToolFail pattern still warns on any tool",
...
test(
    "STALE_FIX_SECONDS is 1200 (20 min)",
...
test(
    "Gate 6 removes stale verified fixes",
    len(_g6_state["verified_fixes"]) == 1
    and "/tmp/fresh_fix.py" in _g6_state["verified_fixes"],
...
code, msg = _direct(
    _g07_check(
        "Write",
        {"file_path": "/tmp/auth_handler.py", "content": "test"},
        {
            "memory_last_queried": time.time() - 350,
            "files_read": ["/tmp/auth_handler.py"],
        },
    )
)
test(
    "Gate 7: write auth_handler.py with stale memory → blocked",
    code != 0,
    f"code={code}",
)
...
code, msg = _direct(
    _g07_check(
        "Edit",
        {"file_path": "/tmp/regular_utils.py"},
...
test(
    "Gate 7 CRITICAL_PATTERNS are (regex, category) tuples",
...
code_g7, msg_g7 = _direct(
    _g07_check(
        "Write",
        {"file_path": f"{_HOME}/.claude/hooks/enforcer.py", "content": "test"},
        {
            "memory_last_queried": time.time() - 350,
            "files_read": [f"{_HOME}/.claude/hooks/enforcer.py"],
        },
    )
)
test(
    "Gate 7 block message includes category",
...
for _pat, _cat in G7_PATTERNS:
    if _re.search(_pat, "/home/user/.ssh/id_rsa", _re.IGNORECASE):
        ...
        break
test(
    "Gate 7 recognizes SSH directory path",
...
code_g7nc, _ = _direct(
    _g07_check(
        "Edit",
        {"file_path": "/tmp/g7_normal232.py"},
        {"memory_last_queried": time.time(), "files_read": ["/tmp/g7_normal232.py"]},
    )
)
test(
    "Gate 7 allows non-critical file",
    code_g7nc == 0,
...
code, msg = _direct(
    _g05_check("Edit", {"file_path": os.path.join(hooks_dir, "enforcer.py")}, _st_h4)
)
...
state = load_state(session_id=MAIN_SESSION)
test(
    "M1: verified_fixes capped at 100",
    len(state["verified_fixes"]) <= 100,
...
state = load_state(session_id=MAIN_SESSION)
test(
    "M2: pending_verification capped at 50",
    len(state["pending_verification"]) <= 50,
...
test(
    "M8: curl does not clear pending verification",
...
test(
    "M8: python clears targeted pending verification",
...
_g6_err_state = {
    ...
    "files_read": ["/tmp/gate6_err.py"],
    "memory_last_queried": time.time(),
    "verified_fixes": [],
    "pending_chain_ids": [],
    "gate6_warn_count": 0,
}
code, msg = _direct_stderr(
    _g06_check, "Edit", {"file_path": "/tmp/gate6_err.py"}, _g6_err_state
)
test(
    "Gate 6 enhanced: warns on unlogged_errors",
    "error" in msg.lower() or "unlogged" in msg.lower(),
    msg,
)
...
_g6_both_result = _g06_check(
    "Edit", {"file_path": "/tmp/gate6_both.py"}, _g6_both_state
)
test(
    "Gate 6 enhanced: blocks when verified_fixes at threshold",
    _g6_both_result.blocked,
    f"blocked={_g6_both_result.blocked}",
)
...
_g6_noblock_state = {
    ...
    "files_read": ["/tmp/gate6_noblock.py"],
    "memory_last_queried": time.time(),
    "verified_fixes": [],
    "pending_chain_ids": [],
}
code, msg = _direct_stderr(
    _g06_check, "Edit", {"file_path": "/tmp/gate6_noblock.py"}, _g6_noblock_state
)
...
_g6_pattern_state = {
    "unlogged_errors": [
        {"pattern": "npm ERR!", "command": "npm install", "timestamp": time.time()}
    ],
    "files_read": ["/tmp/gate6_pattern.py"],
    "memory_last_queried": time.time(),
    "verified_fixes": [],
    "pending_chain_ids": [],
    "gate6_warn_count": 0,
...
code, msg = _direct_stderr(
    _g06_check, "Edit", {"file_path": "/tmp/gate6_pattern.py"}, _g6_pattern_state
)
test(
    "Gate 6 enhanced: warning mentions error pattern",
    "npm ERR!" in msg or "npm" in msg.lower(),
    msg,
)
...
code, msg = _direct(
    _g09_check(
        ...
        {"file_path": "/tmp/g9_test.py"},
        {
            "current_strategy_id": "try-different-import",
            "active_bans": ["some-other-strategy"],
        },
    )
)
...
code, msg = _direct(
    _g09_check(
        ...
        {"file_path": "/tmp/g9_test.py"},
        {
            "current_strategy_id": "reinstall-package",
            "active_bans": ["reinstall-package", "other-ban"],
        },
    )
)
...
code, msg = _direct(
    _g09_check(
        ...
        {
            "current_strategy_id": "reinstall-package",
            "active_bans": ["reinstall-package"],
        },
    )
)
test(
    "Gate 9: Bash with banned strategy → allowed (only blocks Edit/Write)",
    code == 0,
    msg,
)
...
test(
    "_ban_severity(1) → ('first_fail', 'warn')",
...
test(
    "_ban_severity(2) → ('repeating', 'error')",
...
test(
    "_ban_severity(3) → ('escalating', 'critical')",
...
test(
    "_ban_severity(5) → ('escalating', 'critical') — high count",
...
_g9_state9["active_bans"] = {
    "fix-auth": {
        "fail_count": 1,
        "first_failed": time.time() - 60,
        "last_failed": time.time() - 30,
    }
}
...
test(
    "Gate 9 warning shows retry budget",
...
_g9_state10["active_bans"] = {
    "fix-auth": {
        "fail_count": 2,
        "first_failed": time.time() - 120,
        "last_failed": time.time() - 10,
    }
}
...
test(
    "Gate 9 warning at fail_count=2 shows 1 remaining",
...
_g9_state11["active_bans"] = {
    "fix-auth": {
        "fail_count": 3,
        "first_failed": time.time() - 600,
        "last_failed": time.time() - 120,
    }
}
_g9_result11 = gate9_check("Edit", {"file_path": "/tmp/x.py"}, _g9_state11)
test(
    "Gate 9 block includes timing info",
    _g9_result11.blocked
    and "first:" in _g9_result11.message
    and "last:" in _g9_result11.message,
...
test(
    "Gate 9 passes with empty strategy",
...
test(
    "Gate 9 ban threshold constants are correct",
...
test(
    "Gate 9 allows through at fail_count=1 with successes",
...
_g6_chain = {
    "pending_chain_ids": ["chain_abc"],
    "files_read": ["/tmp/g6_chain.py"],
    "memory_last_queried": time.time(),
    "verified_fixes": [],
    "unlogged_errors": [],
    "gate6_warn_count": 0,
...
code, msg = _direct_stderr(
    _g06_check, "Edit", {"file_path": "/tmp/g6_chain.py"}, _g6_chain
)
test(
    "Gate 6: warns on pending_chain_ids",
    "without recorded outcome" in msg or "record_outcome" in msg,
    msg,
)

# 17. Gate 6 pending chain warning mentions record_outcome
test(
    "Gate 6: pending chain warning mentions record_outcome",
    "record_outcome" in msg,
    msg,
)
...
test(
    "Integration: banned strategy blocked by Gate 9",
    code != 0,
    f"code={code}, msg={msg}",
)
...
code, msg = _direct(
    _g03_check(
        "Bash",
        {"command": "scp app.py root@10.0.0.1:/opt/"},
        {"last_test_run": time.time(), "last_test_exit_code": 1},
    )
)
test(
    "M4: deploy after failing tests (exit_code=1) → blocked", code != 0, f"code={code}"
)
...
test(
    "M4: exit code captured from dict tool_response",
...
test(
    "get_memory: updates memory_last_queried",
...
test("Gate14: non-framework file without tests → allowed", not _g14_r1b.blocked)
...
test("Gate14: all signals pass → allowed", not _g14_r4.blocked)
test("Gate14: all signals pass → no warning message", not _g14_r4.message)
...
test(
    "Gate14: re-edit of pending file → allowed (iteration exemption)",
    not _g14_r5.blocked,
)
...
_g14_greenfield_path = (
    "/tmp/_gate14_test_nonexistent_file_" + str(int(time.time())) + ".py"
)
_g14_r_gf = _g14_check("Write", {"file_path": _g14_greenfield_path}, _g14_state5)
test(
    "Gate14: greenfield Write to new file → allowed despite pending",
...
if _g15_check:
    ...
    test(
        "Gate15: test failure + no fix_history → BLOCKED",
...
test("Tracker: test pass clears fixing_error", _tracker_s2.get("fixing_error") is False)
...
try:
    ...
    _r = g05_check(
        "Edit", {"file_path": "/tmp/foo.py"}, _g05_state, event_type="PostToolUse"
    )
    test(
        "G05 Refactored: non-PreToolUse passes", not _r.blocked, f"blocked={_r.blocked}"
    )
    ...
    test(
        "G05 Refactored: _spec test file exempt",
        not _r.blocked,
    ...
    test(
        "G05 Refactored: blocks at 5 unverified files (above threshold 3)",
        _r.blocked is True,
        f"blocked={_r.blocked}",
    )
    ...
    test(
        "G05 Refactored: editing pending file ok (only 2 others)",
        not _r.blocked,
        f"blocked={_r.blocked}",
    )
    ...
    _r = g05_check(
        "NotebookEdit", {"notebook_path": "/home/user/test_nb.ipynb"}, _g05_state
    )
    test(
        "G05 Refactored: notebook_path extracted",
        not _r.blocked,
        f"blocked={_r.blocked}",
    )
    ...
    test(
        "G05 Refactored: non-dict tool_input safe",
        not _r.blocked,
        f"blocked={_r.blocked}",
    )
    ...
    test(
        "G05 Refactored: edit streak warning at 4",
        not _r.blocked,
        f"blocked={_r.blocked}",
    )
    ...
    test(
        "G05 Refactored: edit streak blocks at 6",
        _r.blocked is True,
        f"blocked={_r.blocked}",
    )
...
try:
    ...
    test(
        "Gate 04 refactored: returns GateResult for stale memory",
        hasattr(_g04_r2, "blocked") and hasattr(_g04_r2, "gate_name"),
    )
    ...
    _g04_r6 = g04_check(
        "Edit", {"file_path": "/tmp/test.py"}, _g04_state2, event_type="PostToolUse"
    )
    ...
    test(
        "Gate 07 refactored: allows critical file with recent memory",
        not _g07_r.blocked,
    )
    ...
    test(
        "Gate 07 refactored: returns GateResult for stale memory",
        hasattr(_g07_r2, "blocked") and hasattr(_g07_r2, "gate_name"),
    )
    ...
    # PostToolUse passes
    _g13_r4 = g13_check(
        "Edit", {"file_path": "/tmp/test.py"}, _g13_state2, event_type="PostToolUse"
    )
    test("Gate 13 refactored: PostToolUse passes", not _g13_r4.blocked)

    # Co-claim: two sessions can edit same file when co-claim registered
    import tempfile as _g13_tempfile

    _g13_claims_file = _g13_tempfile.mktemp(suffix=".json")
    _g13_coclaims_file = _g13_tempfile.mktemp(suffix=".json")
    _g13_coclaims = {
        "/tmp/shared_file.py": {
            "sessions": ["session-A", "session-B"],
            "authorized_at": _g04_time.time(),
            "expires": _g04_time.time() + 3600,
        }
    }
    with open(_g13_coclaims_file, "w") as _f:
        import json as _g13_json

        _g13_json.dump(_g13_coclaims, _f)
    _g13_claims = {
        "/tmp/shared_file.py": {
            "session_id": "session-A",
            "claimed_at": _g04_time.time(),
        }
    }
    ...
    _g13_mod.CLAIMS_FILE = _g13_claims_file
    _g13_mod.COCLAIMS_FILE = _g13_coclaims_file
    try:
        _g13_r5 = _g13_mod.check("Edit", _g13_tool_input, _g13_state_b)
        test("Gate 13 co-claim: allows shared edit", not _g13_r5.blocked)
        test(
            "Gate 13 co-claim: message contains 'Co-claim'",
            "Co-claim" in (_g13_r5.message or ""),
        )
    finally:
        _g13_mod.CLAIMS_FILE = _g13_old_claims
        _g13_mod.COCLAIMS_FILE = _g13_old_coclaims
        import os as _g13_os

        _g13_os.unlink(_g13_claims_file)
        _g13_os.unlink(_g13_coclaims_file)
    ...
    test(
        "Gate 15 refactored: passes when fix_history queried recently",
        not _g15_r3.blocked,
    )
...
try:
    ...
    test(
        "Gate 19: increments consecutive block counter",
        _g19_state1.get("gate19_consecutive_blocks") == 1,
    )
    ...
    test(
        "Gate 19: resets escalation_count on cascade reset",
        _g19_state2.get("mentor_escalation_count") == 0,
    )
    test(
        "Gate 19: resets mentor_last_score to 0.5",
        _g19_state2.get("mentor_last_score") == 0.5,
    )
    test(
        "Gate 19: resets block counter",
        _g19_state2.get("gate19_consecutive_blocks") == 0,
    )
    ...
    test(
        "Gate 19: normal allow resets block counter",
        _g19_state3.get("gate19_consecutive_blocks") == 0,
    )
