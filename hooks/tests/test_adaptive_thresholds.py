...
test(
    "Default has confidence gate",
    "gates.gate_14_confidence_check" in data["gate_thresholds"],
)
...
for gate in IMMUTABLE_GATES:
    ...
    test(
        f"{gate.split('.')[-1]} not tracked",
        gate not in data2.get("gate_thresholds", {}),
    )
...
test(
    "Feedback log capped",
    len(data7["feedback_log"]) <= 100 + 5,  # +buffer for new entries
...
with tempfile.TemporaryDirectory() as tmpdir:
    ...
    try:
        ...
        test(
            "Round-trip preserves TP",
            loaded["gate_thresholds"]["gates.gate_14_confidence_check"][
                "true_positives"
            ]
            == 1,
        )
