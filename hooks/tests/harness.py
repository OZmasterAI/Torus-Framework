...
test.__test__ = False  # Prevent pytest from collecting this as a fixture-needing test
...
def run_enforcer(
    event_type, tool_name, tool_input, session_id=MAIN_SESSION, tool_response=None
):
...
def _post(tool_name, tool_input, state, session_id="main", tool_response=None):
    ...
    _tracker_post(
        tool_name, tool_input, state, session_id=session_id, tool_response=tool_response
    )
    return state
