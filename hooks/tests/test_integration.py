...
test(
    "_extract_test_status returns None with no state",
    ts10 is None,
...
test(
    "_extract_test_status reads passed test",
...
test(
    "_extract_test_status detects failed test",
...
test(
    "_extract_verification_quality returns None with no state",
    vq1 is None,
...
test(
    "_extract_verification_quality reads counts",
...
test(
    "_extract_verification_quality returns None for empty lists",
    vq3 is None,
...
test(
    "_extract_verification_quality with only verified fixes",
...
test(
    "_extract_session_duration returns '1h Xm' format",
...
test(
    "_extract_session_duration returns None for <60s",
    _bd2_dur is None,
...
test(
    "_extract_session_duration returns 'Xm' for <1h",
...
test(
    "_extract_session_duration returns None when no state",
    _bd4_dur is None,
...
test(
    "_extract_gate_blocks is callable",
...
if not MEMORY_SERVER_RUNNING:
    ...
    test(
        "format_results returns correct count", len(_fr_out) == 2, f"got {len(_fr_out)}"
    )
    test(
    ...
    if _sp_coll is None:
        ...
        if _sp_result.get("clusters"):
            ...
            test(
                "suggest_promotions cluster has suggested_rule",
                "suggested_rule" in _sp_cluster,
                f"keys={list(_sp_cluster.keys())}",
            )
            test(
                "suggest_promotions cluster has supporting_ids",
                "supporting_ids" in _sp_cluster
                and isinstance(_sp_cluster["supporting_ids"], list),
                f"keys={list(_sp_cluster.keys())}",
            )
            test(
                "suggest_promotions cluster has count",
                "count" in _sp_cluster and isinstance(_sp_cluster["count"], int),
                f"keys={list(_sp_cluster.keys())}",
            )
            test(
                "suggest_promotions cluster has score",
                "score" in _sp_cluster
                and isinstance(_sp_cluster["score"], (int, float)),
                f"keys={list(_sp_cluster.keys())}",
            )
            test(
                "suggest_promotions cluster has avg_age_days",
                "avg_age_days" in _sp_cluster
                and isinstance(_sp_cluster["avg_age_days"], (int, float)),
                f"keys={list(_sp_cluster.keys())}",
            )
            # Verify scoring formula: score = (count * 2) + recency_bonus
            # recency_bonus = max(0, 1 - avg_age/365), so score >= count * 2
            test(
                "suggest_promotions score >= count*2 (formula check)",
            ...
            if len(_sp_result["clusters"]) > 1:
                ...
                test(
                    "suggest_promotions clusters sorted by score desc",
                    _scores == sorted(_scores, reverse=True),
                    f"scores={_scores}",
                )
            # Verify top_k is respected
            test(
                "suggest_promotions respects top_k=3",
...
test(
    "auto-commit: stage() stages file inside ~/.claude/",
...
sys.stdin = _ac_old_stdin
test(
    "auto-commit: stage() skips file outside ~/.claude/",
...
sys.stdin = _ac_old_stdin
test(
    "auto-commit: stage() handles empty tool_input",
    len(_ac_staged_calls) == 0,
    f"calls: {_ac_staged_calls}",
)
...
with open(auto_commit.STAGED_TRACKER, "w") as _ac_f:
    _ac_f.write(
        f"{_HOME}/.claude/hooks/auto_commit.py\n{_HOME}/.claude/hooks/test_framework.py\n"
    )
auto_commit.commit()
test(
    "auto-commit: commit() commits when changes staged",
...
auto_commit.commit()
test(
    "auto-commit: commit() no-ops when nothing staged",
...
if _ac_commit_args:
    ...
    _ac_msg_ok = (
        "boot.py" in _ac_msg
        and "enforcer.py" in _ac_msg
        and "Co-Authored-By" in _ac_msg
    )
test(
    "auto-commit: commit message has file names + co-author",
    _ac_msg_ok,
...
test(
    "Status gather: exits cleanly",
    _bsg_status.returncode == 0,
    f"rc={_bsg_status.returncode}, stderr={_bsg_status.stderr[:200]}",
)
test(
    "Status gather: contains box drawing",
    "\u2554" in _bsg_status.stdout and "\u255d" in _bsg_status.stdout,
...
try:
    ...
    _bsg_wj = {}
    test(
        "Wrap-up gather: valid JSON",
        False,
        f"parse error: {e}, out={_bsg_wrapup.stdout[:100]}",
    )
...
test(
    "PRP: template has Success Criteria section", "## Success Criteria" in _prp_tmpl_src
)
test("PRP: template has Known Gotchas section", "## Known Gotchas" in _prp_tmpl_src)
test(
    "PRP: template has Validation Gates section", "## Validation Gates" in _prp_tmpl_src
)
test(
    "PRP: template has Implementation Tasks section",
    "## Implementation Tasks" in _prp_tmpl_src,
)

# Template is valid markdown (no unclosed code blocks)
_prp_fence_count = _prp_tmpl_src.count("```")
test(
    "PRP: template has balanced code fences",
    _prp_fence_count % 2 == 0,
...
test(
    "PRP: examples README exists",
    os.path.isfile(os.path.join(_prp_examples, "README.md")),
)
...
test(
    "Browser: agent-browser CLI is installed",
    _agent_browser_path is not None,
...
test(
    "TaskManager: status shows 3 tasks",
    _tm_status.get("total") == 3,
    f"total={_tm_status.get('total')}",
)
test(
    "TaskManager: status shows 3 pending",
    _tm_status.get("counts", {}).get("pending") == 3,
)
...
test(
    "TaskManager: next returns task 1 (not 2, blocked by dep)",
    _tm_next.get("id") == 1,
    f"id={_tm_next.get('id')}",
)
...
test(
    "TaskManager: task 1 status is passed",
    _tm_after_update["tasks"][0]["status"] == "passed",
)
...
test(
    "TaskManager: next returns task 2 or 3 after task 1 passed",
    _tm_next2.get("id") in (2, 3),
    f"id={_tm_next2.get('id')}",
)
...
test(
    "TaskManager: validate passing cmd exits 0",
    _tm_r.returncode == 0,
    f"rc={_tm_r.returncode}",
)
with open(_tm_test_file) as _f:
    _tm_after_validate = json.load(_f)
test(
    "TaskManager: validate sets task 2 to passed",
...
test(
    "TaskManager: validate failing cmd exits 1",
    _tm_r.returncode == 1,
    f"rc={_tm_r.returncode}",
)
with open(_tm_test_file) as _f:
...
test(
    "TaskManager: validate sets task 3 to failed",
...
test(
    "TaskManager: template has tasks array",
    "tasks" in _tm_tmpl and isinstance(_tm_tmpl["tasks"], list),
)
...
test(
    "TorusLoop: prompt has validate_command placeholder",
    "{validate_command}" in _ml_prompt_src,
)
test(
...
test(
    "TorusLoop: uses --dangerously-skip-permissions",
    "--dangerously-skip-permissions" in _ml_src,
)
test(
    "TorusLoop: has activity log", "activity.md" in _ml_src or "ACTIVITY_LOG" in _ml_src
)
...
sys.path.insert(0, os.path.join(os.path.expanduser("~"), ".claude", "hooks"))
from memory_server import (
    _parse_transcript_actions,
    _format_teammate_summary,
...
_t1_lines = [
    ...
    _assistant_tool_msg(
        "Edit",
        {
            "file_path": f"{_HOME}/hooks/gate_13.py",
            "old_string": "x",
            "new_string": "y",
        },
    ),
...
_t3_result = _parse_transcript_actions(
    "/tmp/nonexistent_transcript_99999.jsonl", max_actions=5
)
...
_t5_lines = [
    _assistant_tool_msg("Read", {"file_path": f"/tmp/file_{i}.py"}) for i in range(10)
]
...
test(
    "TranscriptParse: text-only messages extracted",
    len(_t6_result) == 2 and "Text:" in _t6_result[0]["action"],
)
os.remove(_t6_path)
...
test(
    "FormatSummary: has numbered list", "  1." in _t7_summary and "  2." in _t7_summary
)

# Test 8: _format_teammate_summary — respects char budget
_t8_actions = [
    {
        "action": f"Read: {_HOME}/some/very/long/path/file_{i}.py with extra detail padding",
        "outcome": "",
    }
    for i in range(20)
]
...
_t9_fd, _t9_state_path = tempfile.mkstemp(
    prefix="state_",
    suffix=".json",
            ...
            "active_subagents": [
                {
                    "agent_id": "abc-123",
                    "agent_type": "builder",
                    "transcript_path": _t10_transcript,
                    "start_ts": time.time(),
                },
                {
                    "agent_id": "def-456",
            ...
            "active_subagents": [
                {
                    "agent_id": "ghi-789",
                    "agent_type": "auditor",
                    "transcript_path": "/tmp/does_not_exist_99999.jsonl",
                    "start_ts": time.time(),
                },
            ]
        },
...
test(
    "GetContext: missing transcript gives graceful summary",
    _t11_result["count"] == 1 and "no actions recorded" in _t11_result["teammates"][0],
)
...
test(
    "GetContext: returns dict with teammates and count keys",
    isinstance(_t12_result, dict)
    and "teammates" in _t12_result
    and "count" in _t12_result,
)
...
if _citation_imports_ok:
    ...
    _c1 = _extract_citations(
        "Found fix [source: https://github.com/foo/bar] in repo", ""
    )
    test(
        "Citation: [source:] extracts primary",
        _c1["primary_source"] == "https://github.com/foo/bar",
    )
    test(
        "Citation: [source:] stripped from content",
        "[source:" not in _c1["clean_content"],
    )

    # Test 2: [ref: URL] extracts reference, strips marker
    _c2 = _extract_citations("See [ref: https://docs.python.org/3/lib] for details", "")
    test(
        "Citation: [ref:] extracts reference",
        "https://docs.python.org/3/lib" in _c2["related_urls"],
    )
    ...
    test(
        "Citation: multiple refs captured",
        "https://github.com/a" in _c3["related_urls"]
        and "https://dev.to/b" in _c3["related_urls"],
    )
    ...
    test(
        "Citation: mixed explicit+auto",
        _c4["primary_source"] == "https://github.com/main"
        and "stackoverflow.com" in _c4["related_urls"],
    )
    ...
    test(
        "Citation: auto-ranking promotes high authority",
        _c5["primary_source"] == "https://github.com/repo",
    )

    # Test 6: Invalid URLs rejected
    test(
        "Citation: validate_url rejects no scheme", _validate_url("not-a-url.com") == ""
    )
    test("Citation: validate_url rejects no netloc", _validate_url("http://") == "")
    test(
        "Citation: validate_url rejects no dot", _validate_url("http://localhost") == ""
    )

    # Test 7: Trailing punctuation stripped
    test(
        "Citation: trailing punctuation stripped",
        _validate_url("https://example.org/page.") == "https://example.org/page",
    )
    test(
        "Citation: trailing paren stripped",
        _validate_url("https://example.org/page)") == "https://example.org/page",
    )
    ...
    test(
        "Citation: empty content no crash",
        _c8["source_method"] == "none" and _c8["primary_source"] == "",
    )

    # Test 9: Malformed marker → fallback to auto
    _c9 = _extract_citations("See [source: broken and https://github.com/fallback", "")
    test(
        "Citation: malformed marker falls back",
        _c9["primary_source"] == "https://github.com/fallback",
    )
    ...
    test(
        "Citation: dedup across content+context",
        _c10["related_urls"].count("https://github.com/repo") <= 1,
    )
    ...
    _total_urls = (1 if _c11["primary_source"] else 0) + len(
        [u for u in _c11["related_urls"].split(",") if u.strip()]
    )
    test("Citation: cap at MAX_CITATION_URLS", _total_urls <= 4)

    # Test 12: Noise URL filtering (localhost, example.com skipped)
    _c12 = _extract_citations(
        "See http://localhost:3000/api and https://github.com/real", ""
    )
    test(
        "Citation: noise URLs filtered",
        "localhost" not in _c12["primary_source"]
        and "localhost" not in _c12["related_urls"],
    )

    # Test 13: source_method values
    _c13a = _extract_citations("[source: https://github.com/x] content here", "")
    test(
        "Citation: source_method=explicit for markers",
        _c13a["source_method"] == "explicit",
    )
    ...
    test(
        "Citation: github.com is high authority",
        _rank_url_authority("https://github.com/x") == 1,
    )
    test(
        "Citation: medium.com is medium authority",
        _rank_url_authority("https://medium.com/x") == 2,
    )
    test(
        "Citation: localhost is low authority",
...
if not MEMORY_SERVER_RUNNING:
    ...
    from memory_server import (
        remember_this as _hl_remember,
        search_knowledge as _hl_search,
    ...
    _hl_problem = _hl_remember(
        ...
        "hybrid linking test",
        "type:error,area:testing,link-test",
    )
    ...
    _hl_fix = _hl_remember(
        ...
        "hybrid linking test",
        f"type:fix,area:testing,link-test,resolves:{_hl_problem_id}",
    )
    ...
    _hl_target_tags = (
        _hl_target_meta["metadatas"][0].get("tags", "")
        if _hl_target_meta.get("metadatas")
        else ""
    )
    test(
    ...
    _hl_organic_fix = any(
        r.get("id") == _hl_fix_id and not r.get("linked")
        for r in _hl_search_result.get("results", [])
    )
    test(
    ...
    _hl_bad_link = _hl_remember(
        ...
        "hybrid linking test",
        "type:fix,area:testing,link-test,resolves:FAKE_NONEXISTENT_ID_12345",
    )
    test(
    ...
    _hl_no_resolve = _hl_remember(
        ...
        "hybrid linking test",
        "type:fix,area:testing,link-test",
    )
    test(
    ...
    test(
        ...
        _hl_multi.get("link_warning") is not None
        and "Multiple" in (_hl_multi.get("link_warning") or ""),
    ...
    _hl_long_tags = "type:error,area:testing," + ",".join(
        f"filler-tag-{i:03d}" for i in range(30)
    )
    ...
    _hl_overflow_fix = _hl_remember(
        ...
        "hybrid linking test",
        f"type:fix,area:testing,link-test,resolves:{_hl_long_id}",
    )
    ...
    _hl_overflow_ok = _hl_overflow_fix.get("linked_to") == _hl_long_id or (
        _hl_overflow_fix.get("link_warning") is not None
        and "overflow" in (_hl_overflow_fix.get("link_warning") or "").lower()
    )
    test(
    ...
    _hl_dedup_search = _hl_search(
        "LINK TEST FIX Gate 99 deadlock lock acquisition", top_k=15
    )
    ...
    test(
        ...
        _hl_bad_link.get("result") == "Memory stored successfully!"
        and "id" in _hl_bad_link,
...
# 1. Secret detection
_g16_r = _g16(
    "Edit", {"file_path": "/tmp/app.py", "new_string": 'api_key = "sk-abc123def456789"'}
)
test(
    "G16: secret in code → warns",
    _g16_r.message is not None and "secret-in-code" in _g16_r.message,
)

# 2. Debug print
_g16_r = _g16(
    "Edit", {"file_path": "/tmp/app.py", "new_string": '    print("debug value")\n'}
)
test(
    "G16: debug print → warns",
    _g16_r.message is not None and "debug-print" in _g16_r.message,
)

# 3. Broad except
_g16_r = _g16(
    "Edit",
    {"file_path": "/tmp/app.py", "new_string": "try:\n    pass\nexcept:\n    pass"},
)
test(
    "G16: broad except → warns",
    _g16_r.message is not None and "broad-except" in _g16_r.message,
)

# 4. TODO detection
_g16_r = _g16(
    "Edit", {"file_path": "/tmp/app.py", "new_string": "# TODO fix this later"}
)
test(
    "G16: TODO → warns (informational)",
    _g16_r.message is not None and "todo-fixme" in _g16_r.message,
)

# 5. Clean code — no warning
_g16_r = _g16(
    "Edit",
    {"file_path": "/tmp/app.py", "new_string": 'def hello():\n    return "world"'},
)
...
_g16_r = _g16(
    "Edit",
    {"file_path": "/tmp/prog.py", "new_string": 'password = "supersecretvalue123"'},
    _g16_state,
)
...
_g16_r = _g16(
    "Edit", {"file_path": "/tmp/reset.py", "new_string": "x = 42"}, _g16_state
)
test(
    "G16: clean edit resets counter",
    _g16_state["code_quality_warnings_per_file"].get("/tmp/reset.py") is None,
)
...
_g16_r = _g16(
    "Edit", {"file_path": "/tmp/config.json", "new_string": '"password": "abc12345678"'}
)
...
_g16_r = _g16(
    "Edit",
    {"file_path": f"{_HOME}/.claude/skills/foo.py", "new_string": 'print("debug")'},
)
...
_g16_r = _g16(
    "NotebookEdit",
    {"notebook_path": "/tmp/nb.py", "new_source": "import pdb\npdb.set_trace()"},
)
test(
    "G16: NotebookEdit debug → warns",
    _g16_r.message is not None and "debug-print" in _g16_r.message,
)
...
_g16_r = _g16(
    "Edit",
    {
        ...
        "new_string": 'api_key = "sk-abc123def456"\nexcept:\n    pass',
    },
)
test(
    "G16: multiple violations in one warning",
    _g16_r.message is not None
    and "secret-in-code" in _g16_r.message
    and "broad-except" in _g16_r.message,
)
...
for _i in range(5):
    _g16(
        "Edit",
        {"file_path": "/tmp/todo.py", "new_string": "# TODO something"},
        _g16_state,
    )
_g16_r = _g16(
    "Edit", {"file_path": "/tmp/todo.py", "new_string": "# FIXME urgent"}, _g16_state
)
test("G16: TODO never escalates (5 TODOs, still not blocked)", _g16_r.blocked is False)

# ── G16 ruff + AST complexity new tests ──────────────────────────────────────
# 16. ruff: unused import in Python file → warns (F401 escalates)
_g16_ruff_content = "import os\nimport sys\nx = 1\n"
_g16_r = _g16(
    "Write", {"file_path": "/tmp/g16_unused_imports.py", "content": _g16_ruff_content}
)
test(
    "G16-ruff: unused import → warns (F401)",
    _g16_r.message is not None and "ruff:F401" in _g16_r.message,
)

# 17. ruff: clean Python → no ruff violation added
_g16_r = _g16(
    "Write",
    {"file_path": "/tmp/g16_clean.py", "content": "def hello():\n    return 42\n"},
)
test(
    "G16-ruff: clean Python → no ruff warning",
    not _g16_r.blocked
    and (_g16_r.message is None or "ruff" not in (_g16_r.message or "").lower()),
)

# 18. ruff_enabled=False skips ruff entirely
_g16_noruff_state = default_state()
_g16_noruff_state["gate_tune_overrides"] = {
    "gate_16_code_quality": {"ruff_enabled": False}
}
_g16_r = _g16(
    "Write",
    {"file_path": "/tmp/g16_noruff.py", "content": "import os\nx = 1\n"},
    _g16_noruff_state,
)
test(
    "G16-ruff: ruff_enabled=False → no ruff violation",
    not (_g16_r.message and "ruff" in _g16_r.message.lower()),
)

# 19. AST complexity: high-complexity function → escalates counter
_g16_complex = "def tangled(a, b, c):\n" + "".join(
    f"    if a > {i}:\n        pass\n" for i in range(22)
)
_g16_r = _g16("Write", {"file_path": "/tmp/g16_complex.py", "content": _g16_complex})
test(
    "G16-complexity: high-complexity fn (>20) → warns",
    _g16_r.message is not None and "complexity:" in _g16_r.message,
)

# 20. AST complexity: simple function → no complexity warning
_g16_r = _g16(
    "Write",
    {
        "file_path": "/tmp/g16_simple.py",
        "content": "def add(a, b):\n    return a + b\n",
    },
)
test(
    "G16-complexity: simple fn → no complexity warning",
    _g16_r.message is None or "complexity:" not in (_g16_r.message or ""),
)

# 21. gate_tune_overrides: complexity_block=5 fires at low complexity
_g16_tune_state = default_state()
_g16_tune_state["gate_tune_overrides"] = {
    "gate_16_code_quality": {"complexity_block": 5, "complexity_warn": 3}
}
_g16_low_complex = "def f(x):\n" + "".join(
    f"    if x > {i}:\n        pass\n" for i in range(4)
)
_g16_r = _g16(
    "Write",
    {"file_path": "/tmp/g16_tuned.py", "content": _g16_low_complex},
    _g16_tune_state,
)
test(
    "G16-tune: complexity_block=5 override fires at low complexity",
    _g16_r.message is not None and "complexity:" in _g16_r.message,
)

# 22. Fail-open: syntax error in content → AST returns empty, no crash
_g16_r = _g16(
    "Write", {"file_path": "/tmp/g16_syntax.py", "content": "def f(\n  pass\n"}
)
test(
    "G16-failopen: syntax error → no crash (fail-open)", True
)  # Verifying no exception raised
...
with open(_claude_md_path) as _f:
    _claude_md = _f.read()
test(
    "Lever1: CLAUDE.md has 'failed-approach' in save rule",
    "failed-approach" in _claude_md,
)
test("Lever1: CLAUDE.md has 'preference' in save rule", "preference" in _claude_md)
...
_auto_remember_event(
    "Test memory content for queue write",
    context="test",
    tags="type:test",
    critical=False,
    state=_test_state_ar,
)
...
test(
    "Lever4: Queue write — entry written to .auto_remember_queue.jsonl",
...
for _i in range(15):
    _auto_remember_event(
        f"Rate limit test entry {_i}",
        context="test",
        tags="type:test",
        critical=False,
        state=_test_state_rl,
    )
_rl_count = 0
if os.path.exists(_test_queue):
    with open(_test_queue) as _qf:
...
test(
    "Lever4: Rate limit — only 10 entries written from 15 triggers",
...
test(
    "Lever4 TriggerA: Test pass → queue entry with test info",
...
test(
    "Lever4 TriggerB: Git commit → queue entry",
...
_test_state_tc["recent_test_failure"] = {
    "pattern": "ImportError: no module named foo",
    "timestamp": time.time(),
}
...
test(
    "Lever4 TriggerC: Error fix verified → queue entry (UDS unavailable fallback)",
    ...
    or "Tests passed" in _tc_content,
...
for _i in range(4):
    _hptu(
        "Edit",
        {"file_path": "/tmp/heavy_file.py", "old_string": "a", "new_string": "b"},
        _test_state_td,
        session_id="test-lever4-d",
    )
_td_content = ""
if os.path.exists(_test_queue):
    with open(_test_queue) as _qf:
...
_td_lines = (
    [l for l in _td_content.strip().split("\n") if l.strip()]
    if _td_content.strip()
    else []
)
test(
    "Lever4 TriggerD: Heavy edit (3+ edits) → queue entry",
    "Heavy editing" in _td_content and "heavy_file.py" in _td_content,
    f"queue content: {_td_content[:200]}",
)
test(
    "Lever4 TriggerD: Only one entry on first crossing (not repeated)",
    len(_td_lines) == 1,
...
for _i, _doc in enumerate(_exp_docs_l2):
    ...
    if _meta.get("has_error") == "true" or _meta.get("error_pattern", ""):
        ...
        if _sid:
            _session_success_tools_l2.setdefault(_sid, set()).add(
                _meta.get("tool_name", "")
            )
...
test(
    "Lever2 Criterion1: Standalone error — Bash error NOT promoted (Bash succeeded later in session)",
...
for _i, _doc in enumerate(["Bash: crash", "Edit: fix"]):
    ...
    if _meta.get("has_error") == "true" or _meta.get("error_pattern", ""):
        ...
        if _sid:
            _session_success_tools_l2b.setdefault(_sid, set()).add(
                _meta.get("tool_name", "")
            )

_standalone_l2b = [
    d
    for _, d, m in _session_errors_l2b
    if not (
        m.get("session_id")
        and m.get("tool_name")
        and m["tool_name"] in _session_success_tools_l2b.get(m["session_id"], set())
    )
]
test(
    "Lever2 Criterion1: Truly standalone error IS promotable",
...
test(
    "Lever2 Criterion2: File in 6 sessions → churn promoted",
...
test(
    "Lever2 Criterion3: 'ls -la' repeated 4x → promoted; 'pytest' excluded",
...
try:
    ...
    _h.PASS += 1
    _h.RESULTS.append(
        f"  PASS: session_end.py has telegram integration (plugin {'present' if _tg_exists else 'absent'})"
    )
...
try:
    _boot_content = open(
        os.path.join(_TG_HOOKS_DIR, "boot.py")
    ).read() + _read_pkg_source(_boot_pkg_dir)
    assert "tg_memories" in _boot_content, "boot.py missing tg_memories variable"
    assert "TELEGRAM L2" in _boot_content, (
        "boot.py missing TELEGRAM L2 dashboard section"
    )
...
try:
    _tg_start_hook = os.path.join(
        _TG_CLAUDE_DIR, "integrations", "telegram-bot", "hooks", "on_session_start.py"
    )
    if os.path.isfile(_tg_start_hook):
        ...
        _h.PASS += 1
        _h.RESULTS.append(
            "  PASS: on_session_start.py (skipTest — plugin not installed)"
        )
...
try:
    ...
    assert _toggle_val is False or _toggle_val is True, (
        f"Unexpected toggle type: {type(_toggle_val)}"
    )
    # Test default for missing key
    _toggle_missing = get_live_toggle("nonexistent_toggle_xyz", "default_val")
    assert _toggle_missing == "default_val", (
        f"Expected 'default_val', got {_toggle_missing}"
    )
    _h.PASS += 1
...
try:
    ...
    _se_hptu(
        ...
        _se_tok_state,
        session_id="test_token_est",
    )
    assert _se_tok_state["session_token_estimate"] == 800, (
        f"Read should add 800, got {_se_tok_state['session_token_estimate']}"
    )
    _se_hptu("Bash", {"command": "ls"}, _se_tok_state, session_id="test_token_est")
    assert _se_tok_state["session_token_estimate"] == 2800, (
        f"Read+Bash should be 2800, got {_se_tok_state['session_token_estimate']}"
    )
    _se_hptu(
        "Edit",
    ...
    assert _se_tok_state["session_token_estimate"] == 4300, (
        f"Read+Bash+Edit should be 4300, got {_se_tok_state['session_token_estimate']}"
    )
    # Task should NOT add to session_token_estimate (tracked separately)
    _se_prev = _se_tok_state["session_token_estimate"]
    _se_hptu(
        "Task",
        {"model": "haiku", "description": "test", "subagent_type": "Explore"},
        _se_tok_state,
        session_id="test_token_est",
    )
    assert _se_tok_state["session_token_estimate"] == _se_prev, (
        "Task should not change session_token_estimate"
    )
    _h.PASS += 1
...
try:
    ...
    try:
        handle_pre_tool_use(
            "Edit",
            {
                "file_path": "/tmp/nonexistent_gate_eff_test.py",
                "old_string": "a",
                "new_string": "b",
            },
            _se_eff_state,
    ...
    assert "gate_01_read_before_edit" in _se_eff_data, (
        f"Expected gate_01 in persistent effectiveness, got {_se_eff_data.keys()}"
    )
    assert _se_eff_data["gate_01_read_before_edit"]["blocks"] >= 1, (
...
try:
    ...
    _se_budget_result = g10_check(
        "Task",
        {"model": "opus", "subagent_type": "builder", "description": "test"},
        _se_budget_state,
    )
...
try:
    ...
    _se_metrics = _se_wrapper.complete(
        _se_chain_state, outcome="success", summary="Fixed bug"
    )
    assert _se_metrics["skill"] == "fix"
    assert _se_metrics["step"] == "1/3"
    assert _se_metrics["tokens_est"] == 4000
    assert _se_metrics["tool_calls"] == 7
    _se_mapping = format_chain_mapping(
        "fix bugs", ["fix", "test"], [_se_metrics], 10.0, 7, "success"
    )
...
try:
    ...
    assert isinstance(_se_cfg["budget_degradation"], bool), (
        "budget_degradation must be bool"
    )
    assert isinstance(_se_cfg["session_token_budget"], (int, float)), (
        "session_token_budget must be numeric"
    )
    assert "chain_memory" not in _se_cfg, (
        "chain_memory should be removed from config.json"
    )
    # Verify toggles are NOT in LIVE_STATE.json anymore
    with open(
        os.path.join(os.path.expanduser("~"), ".claude", "LIVE_STATE.json")
    ) as _se_f2:
        _se_live = _se_json.load(_se_f2)
    for _se_key in ("gate_auto_tune", "budget_degradation", "session_token_budget"):
        assert _se_key not in _se_live, (
            f"Toggle {_se_key} should not be in LIVE_STATE.json"
        )
    _h.PASS += 1
...
try:
    ...
    assert _se_cfg_val is True or _se_cfg_val is False, (
        f"Unexpected type: {type(_se_cfg_val)}"
    )
...
try:
    ...
    _resolve_gate_block_outcomes(
        "Edit", {"file_path": "/tmp/test.py"}, _se_resolve_state
    )
    _se_eff_data = load_gate_effectiveness()
...
try:
    ...
    _se_prevent_state["memory_last_queried"] = (
        _block_ts + 30
    )  # Memory queried AFTER block
    _resolve_gate_block_outcomes(
        "Edit", {"file_path": "/tmp/test.py"}, _se_prevent_state
    )
...
try:
    ...
    for _se_field in (
        ...
        assert _se_schema[_se_field]["category"] == "evolve", (
            f"Expected category 'evolve' for {_se_field}"
        )
    _h.PASS += 1
...
try:
    ...
    _at_state["gate_tune_overrides"] = {
        "gate_04_memory_first": {"freshness_window": 600}
    }
    _at_state["memory_last_queried"] = (
        time.time() - 400
    )  # 400s ago — beyond 300 default, within 600 override
...
from gates.gate_04_memory_first import (
    check as _sl_g04,
    WRITE_FRESHNESS_WINDOW as _sl_wfw,
)
...
try:
    ...
    _sl_state1["memory_last_queried"] = (
        time.time() - 400
    )  # stale for Edit (>300s) but memory was queried
    _sl_path1 = "/tmp/_test_gate4_nonexistent_" + str(int(time.time())) + ".py"
    _sl_r1 = _sl_g04("Write", {"file_path": _sl_path1}, _sl_state1)
    assert not _sl_r1.blocked, (
        f"Write to new file with prior memory query should pass, got: {_sl_r1.message}"
    )
    _h.PASS += 1
...
try:
    ...
    _sl_state3["memory_last_queried"] = (
        time.time() - 700
    )  # stale beyond even 600s Write window
    # Use a file that definitely exists
    _sl_r3 = _sl_g04(
        "Write", {"file_path": f"{_HOME}/.claude/hooks/test_framework.py"}, _sl_state3
    )
...
try:
    ...
    _sl_state4["memory_last_queried"] = (
        time.time() - 400
    )  # 400s ago: >300 Edit window, <600 Write window
    _sl_r4 = _sl_g04(
        "Write", {"file_path": f"{_HOME}/.claude/hooks/test_framework.py"}, _sl_state4
    )
    assert not _sl_r4.blocked, (
        f"Write with 400s-old memory should pass (600s window), got: {_sl_r4.message}"
    )
    _h.PASS += 1
...
try:
    ...
    _sl_r5 = _sl_g04(
        "Edit", {"file_path": f"{_HOME}/.claude/hooks/test_framework.py"}, _sl_state5
    )
