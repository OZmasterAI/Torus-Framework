...
entries = [
    {
        "id": "a1",
        "document": "The gate router uses Q-learning to reorder gates by block probability for optimal early exit",
        "tags": "",
    },
    {
        "id": "a2",
        "document": "Gate router Q-learning reorders gates based on their block probability to enable early exit performance",
        "tags": "",
    },
    {
        "id": "a3",
        "document": "Python best practices for testing include pytest fixtures and parametrize decorators",
        "tags": "",
    },
]
merges = find_merge_candidates(entries, threshold=0.4)
test(
    "Found merge group for similar docs", len(merges) >= 1, f"got {len(merges)} groups"
)
if merges:
    test(
        "Merge group has 2 IDs",
        len(merges[0].memory_ids) == 2,
        f"got {merges[0].memory_ids}",
    )
...
test(
    "Single returns empty",
    find_merge_candidates([{"id": "x", "document": "test"}]) == [],
)

print("\n--- Consolidation: generate_merged_content ---")

docs = [
    "The memory system uses SurrealDB for vector storage. It supports semantic search.",
    "SurrealDB is used for vector storage in the memory system. Cosine similarity is the distance metric.",
]
merged = generate_merged_content(docs)
test("Merged content non-empty", len(merged) > 0)
test(
    "Contains unique info",
    "cosine" in merged.lower() or "distance" in merged.lower(),
    f"merged: {merged[:100]}",
)
...
test(
    "Near-duplicate sentences deduped",
    merged2.count("sky is blue") <= 1,
...
test(
    "High-access tier-2 promoted",
    any(p.memory_ids == ["p1"] for p in promos),
    f"promos={[p.memory_ids for p in promos]}",
)
...
archives_ltp = find_archive_candidates(
    entries_archive, ltp_statuses={"ar1": "full", "ar2": "full"}
)
