...
try:
    ...
    test(
        "Mentor: _eval_bash test pass signal",
    ...
    test(
        "Mentor: _eval_bash test fail signal",
    ...
    test(
        "Mentor: _eval_bash error loop detection",
    ...
    test(
        "Mentor: _eval_bash weak verification",
    ...
    _bash_strong = _eval_bash(
        "Bash",
        {"command": "python3 test_framework.py"},
        {},
        {"error_pattern_counts": {}, "edit_streak": {}},
    )
    test(
        "Mentor: _eval_bash strong verification",
    ...
    test(
        "Mentor: _eval_bash skips non-Bash",
        len(_bash_skip) == 0,
        f"got {len(_bash_skip)}",
    )

    # _eval_edit: churn detection
    _edit_churn = _eval_edit(
        "Edit",
    ...
    test(
        "Mentor: _eval_edit churn detection",
    ...
    _edit_nochurn = _eval_edit(
        "Edit", {"file_path": "/tmp/foo.py"}, {}, {"edit_streak": {"/tmp/foo.py": 2}}
    )
    test(
        "Mentor: _eval_edit no churn below threshold",
    ...
    _edit_revert = _eval_edit(
        "Edit",
        {"file_path": "/tmp/foo.py", "old_string": "x" * 100, "new_string": "y"},
        {},
        {"edit_streak": {}},
    )
    test(
        "Mentor: _eval_edit revert detection",
        any(s.name == "possible_revert" for s in _edit_revert),
        f"signals={[(s.name, s.value) for s in _edit_revert]}",
    )

    # _eval_edit: large edit
    _edit_large = _eval_edit(
        "Edit",
        {"file_path": "/tmp/foo.py", "old_string": "x" * 600, "new_string": "y" * 600},
        {},
        {"edit_streak": {}},
    )
    test(
        "Mentor: _eval_edit large edit advisory",
    ...
    test(
        "Mentor: _eval_edit skips non-edit",
        len(_edit_skip) == 0,
        f"got {len(_edit_skip)}",
    )

    # _eval_search: empty results
    _search_empty = _eval_search("Grep", {}, "", {"mentor_signals": []})
    test(
        "Mentor: _eval_search empty results",
    ...
    test(
        "Mentor: _eval_search non-empty results",
    ...
    test(
        "Mentor: _eval_search stuck detection",
    ...
    _v_warn = _compute_verdict(
        [Signal("churn", 0.3, 2.0, "bad"), Signal("ok", 0.5, 1.0, "meh")]
    )
    test(
        "Mentor: verdict warn (0.3 <= score < 0.5)",
        _v_warn.action == "warn",
        f"action={_v_warn.action} score={_v_warn.score:.2f}",
    )

    # _compute_verdict: escalate threshold
    _v_escalate = _compute_verdict(
        [Signal("fail", 0.0, 3.0, "total fail"), Signal("loop", 0.1, 2.0, "stuck")]
    )
    test(
        "Mentor: verdict escalate (score < 0.3)",
    ...
    _eval_v = mentor_evaluate(
        "Bash", {"command": "pytest"}, {"exit_code": 0}, _eval_state
    )
    test(
        "Mentor: evaluate updates state mentor_last_verdict",
        _eval_state.get("mentor_last_verdict") == "proceed",
        f"got {_eval_state.get('mentor_last_verdict')}",
    )
    test(
        "Mentor: evaluate updates state mentor_last_score",
    ...
    _esc_state = {
        "tool_call_count": 1,
        "error_pattern_counts": {"err": 5},
        "edit_streak": {},
        "mentor_signals": [],
        "mentor_escalation_count": 0,
    }
    _esc_v = mentor_evaluate(
        "Bash", {"command": "pytest"}, {"exit_code": 1}, _esc_state
    )
    test(
        "Mentor: escalation counter increments on escalate",
        _esc_state.get("mentor_escalation_count", 0) >= 1
        if _esc_v and _esc_v.action == "escalate"
        else True,
    ...
    _reset_v = mentor_evaluate(
        "Bash", {"command": "pytest"}, {"exit_code": 0}, _reset_state
    )
    test(
        "Mentor: escalation counter resets on proceed",
    ...
    test(
        "Mentor: evaluate fail-open on bad input",
...
try:
    from gates.gate_19_hindsight import (
        check as g19_check,
        GATE_NAME as G19_NAME,
        WATCHED_TOOLS as G19_TOOLS,
    )
    ...
    test(
        "Gate 19: watches Edit/Write/NotebookEdit",
    ...
    _g19r1 = g19_check(
        "Edit", {"file_path": "/tmp/foo.py"}, {}, event_type="PostToolUse"
    )
    test(
        "Gate 19: skips non-PreToolUse", not _g19r1.blocked, f"blocked={_g19r1.blocked}"
    )

    # Skips non-watched tools
    _g19r2 = g19_check("Bash", {"command": "ls"}, {}, event_type="PreToolUse")
    test(
        "Gate 19: skips non-watched tools",
        not _g19r2.blocked,
        f"blocked={_g19r2.blocked}",
    )
    ...
    test(
        "Gate 19: skips when toggle off",
        not _g19r3.blocked,
        f"blocked={_g19r3.blocked}",
    )
    ...
    test(
        "Gate 19: skips when fixing_error=True",
        not _g19r4.blocked,
        f"blocked={_g19r4.blocked}",
    )

    # Skips exempt files (test files)
    _g19r5 = g19_check(
        "Edit",
        {"file_path": "/tmp/test_foo.py"},
        {"mentor_last_score": 0.1, "mentor_escalation_count": 5},
        event_type="PreToolUse",
    )
    test(
        "Gate 19: skips exempt test files",
        not _g19r5.blocked,
        f"blocked={_g19r5.blocked}",
    )

    # Does not read Gate 5 fields (pending_verification, edit_streak)
    _g19_dep_reads = _g19_deps.get("gate_19_hindsight", {}).get("reads", [])
    test(
        "Gate 19: never reads pending_verification",
        "pending_verification" not in _g19_dep_reads,
        f"reads={_g19_dep_reads}",
    )
    test(
        "Gate 19: never reads edit_streak",
    ...
    test(
        "Gate 19: reads fixing_error only to skip",
        "fixing_error" in _g19_dep_reads,
        f"reads={_g19_dep_reads}",
    )
    test(
        "Gate 19: never reads fix_history_queried",
        "fix_history_queried" not in _g19_dep_reads,
        f"reads={_g19_dep_reads}",
    )
    test(
        "Gate 19: never reads recent_test_failure",
    ...
    test(
        "Gate 19: writes no state fields",
    ...
    test(
        "Gate 19: before Gate 11 in registry",
        _g19_idx < _g11_idx,
...
try:
    ...
    _oc_skip = chains_evaluate(
        "Bash",
        {},
        {},
        {"tool_call_count": 7, "tool_call_counts": {}, "total_tool_calls": 20},
    )
    ...
    _oc_state10 = {
        "tool_call_count": 10,
        "tool_call_counts": {"Read": 5, "Edit": 3, "Bash": 2},
        "total_tool_calls": 10,
    }
    ...
    _oc_churn_state = {
        "tool_call_count": 20,
        "tool_call_counts": {"Edit": 10, "Write": 3, "Bash": 2, "Read": 5},
        "total_tool_calls": 20,
    }
    _oc_churn = chains_evaluate("Edit", {}, {}, _oc_churn_state)
    test(
        "Chains: churn detection",
    ...
    test(
        "Chains: updates mentor_chain_pattern in state",
        "mentor_chain_pattern" in _oc_update_state,
        f"keys={list(_oc_update_state.keys())}",
    )
    test(
        "Chains: updates mentor_chain_score in state",
        "mentor_chain_score" in _oc_update_state,
        f"keys={list(_oc_update_state.keys())}",
    )

    # Skips when too few calls
    _oc_low = chains_evaluate(
        "Bash",
        {},
        {},
        {"tool_call_count": 10, "tool_call_counts": {"Read": 3}, "total_tool_calls": 5},
    )
...
try:
    from tracker_pkg.mentor_memory import (
    ...
    _mm_result = mem_evaluate(
        "Bash",
        {"command": "pytest"},
        {},
        {"recent_test_failure": None, "current_strategy_id": ""},
    )
    test(
        "MemMentor: standalone operation (no Module A dependency)",
    ...
    test(
        "MemMentor: fail-open when UDS socket missing",
        _mm_uds is None,
        f"got {_mm_uds}",
    )

    # Context extraction: error pattern
    _mm_ctx1 = _extract_query_context(
        "Bash",
        {},
        {},
        {"recent_test_failure": {"pattern": "ImportError"}, "current_strategy_id": ""},
    )
    test(
        "MemMentor: extracts error pattern context",
        "ImportError" in _mm_ctx1,
        f"got '{_mm_ctx1}'",
    )

    # Context extraction: file path
    _mm_ctx2 = _extract_query_context(
        "Edit",
        {"file_path": "/home/test/foo.py"},
        {},
        {"recent_test_failure": None, "current_strategy_id": ""},
    )
    test(
        "MemMentor: extracts file path context",
        "foo.py" in _mm_ctx2,
        f"got '{_mm_ctx2}'",
    )

    # Context extraction: command
    _mm_ctx3 = _extract_query_context(
        "Bash",
    ...
    test(
        "MemMentor: extracts command context", "pytest" in _mm_ctx3, f"got '{_mm_ctx3}'"
    )

    # Context extraction: strategy
    _mm_ctx4 = _extract_query_context(
        "Edit",
        {},
        {},
        {"recent_test_failure": None, "current_strategy_id": "fix-type-cast"},
    )
    test(
        "MemMentor: extracts strategy context",
        "fix-type-cast" in _mm_ctx4,
        f"got '{_mm_ctx4}'",
    )

    # Context extraction: empty = empty string
    _mm_ctx5 = _extract_query_context(
        "Read", {}, {}, {"recent_test_failure": None, "current_strategy_id": ""}
    )
    test(
        "MemMentor: empty context returns empty string",
        _mm_ctx5 == "",
        f"got '{_mm_ctx5}'",
    )
...
try:
    from shared.state import (
        default_state as _mentor_default_state,
        get_state_schema as _mentor_schema,
    )
    ...
    for _mf in [
        ...
        test(
            f"Mentor integration: {_mf} in state schema",
...
try:
    from tracker_pkg.mentor_analytics import (
        evaluate as _ma_eval,
        _TRIGGERS as _ma_triggers,
    )

    # 1. Gate file edit triggers gate_dashboard nudge
    _ma_state1 = {"total_tool_calls": 10, "analytics_last_used": {}}
    _ma_msgs1 = _ma_eval(
        "Edit", {"file_path": f"{_HOME}/.claude/hooks/gates/gate_04.py"}, {}, _ma_state1
    )
    test(
        "UpgradeC: gate edit triggers gate_dashboard nudge",
    ...
    _ma_msgs2 = _ma_eval(
        "Edit",
        {"file_path": f"{_HOME}/.claude/skills/benchmark/SKILL.md"},
        {},
        _ma_state1,
    )
    test(
        "UpgradeC: skill edit triggers skill_health nudge",
    ...
    _ma_msgs3 = _ma_eval(
        "Edit", {"file_path": f"{_HOME}/.claude/hooks/enforcer.py"}, {}, _ma_state1
    )
    test(
        "UpgradeC: enforcer edit triggers gate_timing nudge",
    ...
    _ma_msgs4 = _ma_eval(
        "Edit", {"file_path": f"{_HOME}/Desktop/app.py"}, {}, _ma_state4
    )
    test(
        "UpgradeC: non-framework edit → no path-based nudge",
        not any(
            "gate_dashboard" in m or "skill_health" in m or "gate_timing" in m
            for m in _ma_msgs4
        ),
        f"msgs={_ma_msgs4}",
    )
    ...
    _ma_state5 = {
        "total_tool_calls": 10,
        "analytics_last_used": {"gate_dashboard": _ma_time.time()},
    }
    _ma_msgs5 = _ma_eval(
        "Edit", {"file_path": f"{_HOME}/.claude/hooks/gates/gate_04.py"}, {}, _ma_state5
    )
    test(
        "UpgradeC: cooldown suppresses nudge after recent analytics call",
    ...
    test(
        "UpgradeC: periodic checkpoint at 50th tool call",
    ...
    _ma_msgs7 = _ma_eval(
        "Read", {"file_path": f"{_HOME}/.claude/hooks/gates/gate_04.py"}, {}, _ma_state7
    )
    test(
        "UpgradeC: Read tool → no nudge",
...
print("\n--- Upgrade F: Gate 6 Analytics Advisory (removed) ---")
test(
    "UpgradeF: analytics counter removed from Gate 6",
    not hasattr(
        __import__(
            "gates.gate_06_save_fix", fromlist=["ANALYTICS_ESCALATION_THRESHOLD"]
        ),
        "ANALYTICS_ESCALATION_THRESHOLD",
    ),
...
try:
    ...
    test(
        "is_analytics_tool: recognises analytics tool",
        is_analytics_tool("mcp__analytics__framework_health") == True,
    )
    test(
        "is_analytics_tool: rejects memory tool",
        is_analytics_tool("mcp__memory__search") == False,
    )
    test("is_analytics_tool: rejects plain tool", is_analytics_tool("Edit") == False)
    test("is_analytics_tool: rejects empty string", is_analytics_tool("") == False)
    test(
        "is_always_allowed: analytics tool is always allowed",
        is_always_allowed("mcp__analytics__session_summary") == True,
    )
    test(
        "is_always_allowed: analytics all_metrics is allowed",
        is_always_allowed("mcp__analytics__all_metrics") == True,
    )
    test(
        "ANALYTICS_TOOL_PREFIX is correct", ANALYTICS_TOOL_PREFIX == "mcp__analytics__"
    )
...
try:
    ...
    _g11a_result = _g11_analytics_check(
        "mcp__analytics__framework_health", {}, _g11a_state
    )
    test("Gate 11: analytics tool → not blocked", not _g11a_result.blocked)
    test(
        "Gate 11: analytics tool → no timestamp appended",
    ...
    _g11b_result = _g11_analytics_check(
        "Edit", {"file_path": "/tmp/test.py"}, _g11b_state
    )
    test(
        "Gate 11: normal tool still appends timestamp",
...
try:
    ...
    test(
        "_resolve_session_id passes explicit ID through",
...
try:
    ...
    test(
        "telegram_search('') returns empty results",
        isinstance(_tg_empty, dict)
        and _tg_empty.get("count") == 0
        and _tg_empty.get("results") == []
        and _tg_empty.get("source") == "telegram_fts",
    )

    # Telegram search: real query → dict with expected keys
    _tg_result = telegram_search("test")
    test(
        "telegram_search('test') returns dict with count/results keys",
        isinstance(_tg_result, dict)
        and "count" in _tg_result
        and "results" in _tg_result
        and "source" in _tg_result,
    )

    # Telegram search: limit clamping → no crash
    _tg_clamp = telegram_search("test", limit=100)
    test(
        "telegram_search limit=100 clamped, no crash",
    ...
    test(
        "terminal_history_search('') returns empty results",
        isinstance(_th_empty, dict)
        and _th_empty.get("count") == 0
        and _th_empty.get("results") == []
        and _th_empty.get("source") == "terminal_fts",
    )

    # Terminal history search: real query → dict with expected keys
    _th_result = terminal_history_search("python")
    test(
        "terminal_history_search('python') returns dict with count/results keys",
        isinstance(_th_result, dict)
        and "count" in _th_result
        and "results" in _th_result
        and "source" in _th_result,
    )

    # Terminal history search: negative limit clamped to 1
    _th_clamp = terminal_history_search("x", limit=-1)
    test(
        "terminal_history_search limit=-1 clamped to 1, no crash",
    ...
    test(
        "transcript_context('') returns error dict",
        isinstance(_tc_empty, dict)
        and "error" in _tc_empty
    ...
    test(
        "transcript_context(nonexistent) returns error or disabled",
        isinstance(_tc_missing, dict)
        and _tc_missing.get("source") == "transcript_l0"
    ...
    if _tc_jsonls:
        ...
        test(
            "transcript_context(real_session) returns records or disabled",
            isinstance(_tc_real, dict)
            and _tc_real.get("source") == "transcript_l0"
...
try:
    _term_hist_dir = os.path.join(
        os.path.expanduser("~"), ".claude", "integrations", "terminal-history"
    )
    if _term_hist_dir not in sys.path:
        sys.path.insert(0, _term_hist_dir)
    from db import (
        _summarize_record,
        _window_around_timestamp,
        get_raw_transcript_window,
    )
    ...
    test(
        "_summarize_record(text msg) extracts role and text",
    ...
    test(
        "_summarize_record(tool_use) extracts tool name",
        _sr_tool.get("content_blocks")
        and _sr_tool["content_blocks"][0].get("name") == "Read",
    )
    ...
    test(
        "_summarize_record(tool_result) extracts output preview",
    ...
    _sr_long = _summarize_record(
        {
            "type": "user",
            "timestamp": "2026-02-25T10:00:03",
            "message": {"role": "user", "content": "x" * 1000},
        }
    )
    test(
        "_summarize_record truncates text at 500 chars",
    ...
    test(
        "_summarize_record(progress) extracts hook_event",
    ...
    _wt_filtered = _window_around_timestamp(
        _wt_records, "2026-02-25T10:05:00", window_minutes=6
    )
    _wt_types = [r["type"] for r in _wt_filtered]
    test(
        "_window_around_timestamp filters ±6min correctly",
        "a" in _wt_types
        and "b" in _wt_types
        and "c" in _wt_types
        and "d" not in _wt_types
        and "e" not in _wt_types,
    )

    # _window_around_timestamp: bad timestamp falls back to last 30
    _wt_fallback = _window_around_timestamp(
        _wt_records, "not-a-timestamp", window_minutes=5
    )
    test(
        "_window_around_timestamp bad timestamp falls back to last records",
        len(_wt_fallback) == len(_wt_records),
    )  # all 5 since < 30

    # get_raw_transcript_window: missing file
    _grw_missing = get_raw_transcript_window("nonexistent-uuid-000")
    test(
        "get_raw_transcript_window(missing) returns error dict",
        isinstance(_grw_missing, dict)
        and "error" in _grw_missing
    ...
    if _grw_jsonls:
        ...
        test(
            "get_raw_transcript_window(real) returns records",
            isinstance(_grw_real, dict)
            and "records" in _grw_real
...
_eb2.clear()
_eb2.subscribe(
    _eb2.EventType.ERROR_DETECTED, lambda e: (_ for _ in ()).throw(RuntimeError("boom"))
)
...
test(
    "RateLimiter: tool: prefix uses TOOL_RATE (10, 10)",
    _rl_tool_cfg == (10.0, 10),
    f"got {_rl_tool_cfg}",
)

_rl_gate_cfg = _config_for("gate:gate_04")
test(
    "RateLimiter: gate: prefix uses GATE_RATE (30, 30)",
    _rl_gate_cfg == (30.0, 30),
    f"got {_rl_gate_cfg}",
)

_rl_api_cfg = _config_for("api:memory")
test(
    "RateLimiter: api: prefix uses API_RATE (60, 60)",
    _rl_api_cfg == (60.0, 60),
    f"got {_rl_api_cfg}",
)
...
test(
    "RateLimiter: reset gives full bucket (10 for tool:)",
    _rl_rem == 10,
    f"got {_rl_rem}",
)
...
test(
    "RateLimiter: remaining decremented after consume",
    _rl_after == 9,
    f"got {_rl_after}",
)
...
test(
    "RateLimiter: allow does not consume tokens",
    _rl_check is True and _rl_still == 10,
    f"check={_rl_check}, rem={_rl_still}",
)

# Test 6: get_all_limits includes tracked keys
_rl_limits = get_all_limits()
test(
    "RateLimiter: get_all_limits returns dict with expected keys",
...
test(
    "RetryStrategy: should_retry True before max failures",
...
test(
    "RetryStrategy: exponential delays [1.0, 2.0, 4.0]",
    _rs_delays == [1.0, 2.0, 4.0],
    f"got {_rs_delays}",
)
...
_rs_const_cfg = RetryConfig(
    strategy=Strategy.CONSTANT, base_delay=3.0, jitter=Jitter.NONE
)
_rs_const_d = rs_get_delay("__test_const__", config=_rs_const_cfg)
test(
    "RetryStrategy: constant delay == base_delay (3.0)",
    abs(_rs_const_d - 3.0) < 1e-9,
    f"got {_rs_const_d}",
)
...
test(
    "RetryStrategy: get_stats has correct attempt/success/failure counts",
    _rs_stats.get("attempts") == 2
    and _rs_stats.get("successes") == 1
...
_rs_jit_cfg = RetryConfig(
    strategy=Strategy.CONSTANT, base_delay=5.0, jitter=Jitter.FULL
)
_rs_jit_vals = [rs_get_delay("__test_jitter__", config=_rs_jit_cfg) for _ in range(20)]
test(
    "RetryStrategy: full jitter delays in [0, 5.0]",
...
test(
    "RetryStrategy: reset clears attempts to 0",
    _rs_reset_stats.get("attempts") == 0,
    f"got {_rs_reset_stats.get('attempts')}",
)
...
test(
    "MemoryDecay: fresh T1 memory scores >= 0.9",
    _md_fresh_score >= 0.9,
    f"got {_md_fresh_score:.3f}",
)
...
test(
    "MemoryDecay: 180-day old T3 memory scores < 0.15",
    _md_old_score < 0.15,
    f"got {_md_old_score:.3f}",
)
...
test(
    "MemoryDecay: decay factor at day 1 > day 30 (monotonic)",
    _md_decay_1 > _md_decay_30,
    f"d1={_md_decay_1:.4f}, d30={_md_decay_30:.4f}",
)
...
test(
    "MemoryDecay: access_boost(10) > access_boost(0)",
...
_md_entries = [
    {
        "tier": 3,
        "timestamp": (datetime.now(tz=timezone.utc) - timedelta(days=100)).isoformat(),
        "retrieval_count": 0,
        "tags": "",
    },
    {
...
test(
    "MemoryDecay: rank_memories sorts descending by relevance",
    len(_md_ranked) == 2
...
test(
    "MemoryDecay: identify_stale_memories finds old low-tier entries",
...
test(
    "MemoryDecay: tag_relevance_bonus > 0 for matching tags",
    _md_tag_bonus > 0,
    f"got {_md_tag_bonus:.3f}",
)
...
test(
    "GateHealth: report has health_score key",
...
test(
    "GateHealth: health_score is int in [0, 100]",
...
test(
    "GateHealth: report has gate_count >= 0",
...
test(
    "GateHealth: slow_gates is a list",
...
test(
    "GateHealth: format_health_dashboard returns string with Score",
...
# ─── Skill Health Tests ──────────────────────────────────────────────
print("\n--- Skill Health ---")
from shared.skill_health import (
    check_all_skills,
    get_broken_skills,
    format_health_report as sh_format,
)

# Test 1: check_all_skills returns expected structure
_sh_report = check_all_skills()
test(
    "SkillHealth: check_all_skills has total_skills key",
...
test(
    "SkillHealth: healthy_skills <= total_skills",
...
test(
    "SkillHealth: get_broken_skills returns list",
    isinstance(_sh_broken, list),
    f"type={type(_sh_broken).__name__}",
)

# Test 5: format_health_report returns string
_sh_formatted = sh_format(_sh_report)
test(
    "SkillHealth: format_health_report returns non-empty string",
...
test(
    "HookProfiler: profile wraps function and returns gate result",
    hasattr(_hp_result, "blocked") and _hp_result.blocked is False,
)

# Test 2: wrapped function has _profiler_wrapped attribute
test(
    "HookProfiler: wrapped function has _profiler_wrapped=True",
...
test(
    "HookProfiler: _percentile returns 0.0 for empty list",
    _hp_p_empty == 0.0,
    f"got {_hp_p_empty}",
)
...
test("HookProfiler: _percentile p50 of [1..10] is 5", _hp_p50 == 5, f"got {_hp_p50}")

# Test 5: analyze returns dict
_hp_stats = analyze()
test(
    "HookProfiler: analyze returns dict",
    isinstance(_hp_stats, dict),
    f"type={type(_hp_stats).__name__}",
)

# Test 6: report returns string
_hp_rpt = hp_report()
test(
    "HookProfiler: report returns string",
    isinstance(_hp_rpt, str),
    f"type={type(_hp_rpt).__name__}",
)
...
test(
    "ToolPatterns: build_markov_chain returns object with transitions",
...
test(
    "ToolPatterns: markov chain vocabulary includes Read, Edit, Bash",
...
test(
    "ToolPatterns: markov chain sequence_count == 3",
    _tp_chain.sequence_count == 3,
    f"got {_tp_chain.sequence_count}",
)

# Test 4: predict_next_tool returns list of tuples
_tp_preds = predict_next_tool(["Read", "Edit"])
test(
    "ToolPatterns: predict_next_tool returns list",
    isinstance(_tp_preds, list),
    f"type={type(_tp_preds).__name__}",
)

# Test 5: get_tool_stats returns dict
_tp_stats = get_tool_stats()
test(
    "ToolPatterns: get_tool_stats returns dict",
    isinstance(_tp_stats, dict),
    f"type={type(_tp_stats).__name__}",
)

# Test 6: summarize_patterns returns dict with expected keys
_tp_summary = summarize_patterns()
test(
    "ToolPatterns: summarize_patterns returns dict with vocabulary_size",
...
from shared.error_pattern_analyzer import (
    extract_pattern,
    analyze_errors,
    top_patterns as ep_top_patterns,
    suggest_prevention,
...
test(
    "ErrorPatternAnalyzer: extract_pattern identifies import error",
    "import" in _ep_import.lower(),
    f"got {_ep_import!r}",
)
...
test(
    "ErrorPatternAnalyzer: extract_pattern identifies gate1 block",
...
test(
    "ErrorPatternAnalyzer: suggest_prevention returns non-empty string",
    isinstance(_ep_sug, str) and len(_ep_sug) > 0,
    f"got {_ep_sug!r}",
)
...
test(
    "ErrorPatternAnalyzer: frequency_from_strings returns dict with counts",
...
test(
    "ErrorPatternAnalyzer: analyze_errors({}) returns dict with total_errors",
...
test(
    "ErrorPatternAnalyzer: correlate_errors({}) returns list",
    isinstance(_ep_corr, list),
    f"type={type(_ep_corr).__name__}",
)
...
test(
    "GateCorrelation: analyze_correlations returns dict with pairs key",
...
test(
    "GateCorrelation: pairs is a list",
...
test(
    "GateCorrelation: gate_block_counts is a dict",
...
test(
    "GateCorrelation: total_events >= 0",
    isinstance(_gc_data.get("total_events"), int) and _gc_data["total_events"] >= 0,
...
test(
    "GateCorrelation: format_correlation_report returns string",
    isinstance(_gc_report, str),
    f"type={type(_gc_report).__name__}",
)
...
test(
    ...
    isinstance(_dr_domains, list),
    f"type={type(_dr_domains).__name__}",
)
...
for _d in _dr_domains[:3]:
    if not all(k in _d for k in ("name", "active", "graduated", "has_mastery")):
        ...
        break
test(
    "DomainRegistry: domain entries have expected keys",
...
test(
    "DomainRegistry: DEFAULT_PROFILE has gate_modes key",
...
test(
    "DomainRegistry: load_domain_profile for missing domain returns dict",
    isinstance(_dr_fake, dict),
    f"type={type(_dr_fake).__name__}",
)
...
test(
    "StateMigrator: migrate_state adds missing fields",
...
test(
    "StateMigrator: migrated state has current STATE_VERSION",
...
test(
    "StateMigrator: default_state() passes validation (ignoring nullable fields)",
...
test(
    "StateMigrator: validate_state rejects non-dict",
...
test(
    "StateMigrator: get_schema_diff finds missing fields",
...
test(
    "StateMigrator: migrate_state on non-dict returns default_state",
...
test(
    "PluginRegistry: scan_plugins returns list",
    isinstance(_pr_plugins, list),
    f"type={type(_pr_plugins).__name__}",
)
...
for _p in _pr_plugins[:3]:
    if not all(k in _p for k in ("name", "version", "category", "enabled", "path")):
        ...
        break
test(
    "PluginRegistry: plugin entries have expected keys",
...
test(
    "PluginRegistry: _infer_category(code-review) -> quality",
    _pr_cat == "quality",
    f"got {_pr_cat!r}",
)
...
test(
    "PluginRegistry: _infer_category(vulnerability-scanner) -> security",
    _pr_sec_cat == "security",
    f"got {_pr_sec_cat!r}",
)

# Test 5: KNOWN_CATEGORIES includes quality, security, development
test(
    "PluginRegistry: KNOWN_CATEGORIES has expected entries",
...
test(
    "PluginRegistry: get_by_category returns list",
    isinstance(_pr_dev, list),
    f"type={type(_pr_dev).__name__}",
)
...
test(
    "PluginRegistry: validate_plugin on missing path returns (False, errors)",
...
test(
    "SessionAnalytics: get_session_summary returns dict",
    isinstance(_sa_summary, dict),
    f"type={type(_sa_summary).__name__}",
)

# Test 2: tool_call_distribution with empty input
_sa_tcd = tool_call_distribution([])
test(
    "SessionAnalytics: tool_call_distribution({}) returns empty dict",
...
test(
    "SessionAnalytics: tool_call_distribution counts correctly",
...
test(
    "SessionAnalytics: gate_fire_rates({}) returns empty dict",
    isinstance(_sa_gfr, dict),
    f"type={type(_sa_gfr).__name__}",
)

# Test 5: session_productivity with empty input
_sa_prod = session_productivity([], 60.0)
test(
    "SessionAnalytics: session_productivity returns dict with score",
...
test(
    "SessionCompressor: compress_session_context({}) returns string",
    isinstance(_sc_compressed, str),
    f"type={type(_sc_compressed).__name__}",
)
...
test(
    "SessionCompressor: compress returns string (may be empty for fresh state)",
...
test(
    "SessionCompressor: extract_key_decisions returns list",
    isinstance(_sc_decisions, list),
    f"type={type(_sc_decisions).__name__}",
)

# Test 4: format_handoff returns string
_sc_handoff = format_handoff(_sc_state, _sc_decisions)
test(
    "SessionCompressor: format_handoff returns string",
    isinstance(_sc_handoff, str),
    f"type={type(_sc_handoff).__name__}",
)
...
test(
    "AgentChannel: post_message returns bool",
    isinstance(_ac_ok, bool),
    f"type={type(_ac_ok).__name__}",
)
...
test(
    "AgentChannel: read_messages returns list",
    isinstance(_ac_msgs, list),
    f"type={type(_ac_msgs).__name__}",
)

# Test 3: cleanup returns int
_ac_cleaned = cleanup(max_age_hours=0)  # Clean everything
test(
    "AgentChannel: cleanup returns int (count deleted)",
    isinstance(_ac_cleaned, int),
    f"type={type(_ac_cleaned).__name__}",
)
...
test(
    ...
    f"type={type(_gp_analysis).__name__}",
)
...
for _g, _a in list(_gp_analysis.items())[:3]:
    if not hasattr(_a, "verdict"):
        ...
        break
test(
    "GatePruner: analysis entries have verdict attribute",
...
test(
    "GatePruner: get_prune_recommendations returns list",
    isinstance(_gp_recs, list),
    f"type={type(_gp_recs).__name__}",
)

# Test 4: render_pruner_report returns string
_gp_report = render_pruner_report()
test(
    "GatePruner: render_pruner_report returns non-empty string",
...
test(
    "GateDashboard: get_gate_metrics returns dict",
    isinstance(_gd_metrics, dict),
    f"type={type(_gd_metrics).__name__}",
)

# Test 2: rank_gates_by_value returns list of tuples
_gd_ranked = rank_gates_by_value()
test(
    "GateDashboard: rank_gates_by_value returns list",
    isinstance(_gd_ranked, list),
    f"type={type(_gd_ranked).__name__}",
)

# Test 3: render_dashboard returns string
_gd_dash = gd_render()
test(
    "GateDashboard: render_dashboard returns string",
    isinstance(_gd_dash, str),
    f"type={type(_gd_dash).__name__}",
)

# Test 4: get_recommendations returns list of strings
_gd_recs = get_recommendations()
test(
    "GateDashboard: get_recommendations returns list",
    isinstance(_gd_recs, list),
    f"type={type(_gd_recs).__name__}",
)
...
test(
    "MetricsExporter: export_json returns dict with metrics key",
...
test(
    "MetricsExporter: export_prometheus returns string",
    isinstance(_me_prom, str),
    f"type={type(_me_prom).__name__}",
)

# Test 3: export_json has exported_at timestamp
test(
    "MetricsExporter: export_json has exported_at field",
...
try:
    ...
    test(
        "ExperienceArchive: record_fix returns bool",
        isinstance(_ea_ok, bool),
        f"type={type(_ea_ok).__name__}",
    )

    # Test 2: get_archive_stats returns dict
    _ea_stats = get_archive_stats()
    test(
        "ExperienceArchive: get_archive_stats returns dict",
        isinstance(_ea_stats, dict),
        f"type={type(_ea_stats).__name__}",
    )

    # Test 3: get_archive_stats has total_rows key
    test(
        "ExperienceArchive: stats has total_rows key",
        "total_rows" in _ea_stats,
        f"keys={set(_ea_stats.keys())}",
    )

    # Test 4: query_best_strategy returns string
    _ea_best = query_best_strategy("ImportError")
    test(
        "ExperienceArchive: query_best_strategy returns string",
        isinstance(_ea_best, str),
        f"type={type(_ea_best).__name__}",
    )

    # Test 5: get_success_rate returns float in [0, 1]
    _ea_rate = get_success_rate("pip_install")
    test(
        "ExperienceArchive: get_success_rate returns float in [0,1]",
...
test(
    "MemoryMaintenance: analyze_memory_health returns dict",
    isinstance(_mm_health, dict),
    f"type={type(_mm_health).__name__}",
)

# Test 2: health report has summary key
test(
    "MemoryMaintenance: health report has summary key",
...
test(
    "MemoryMaintenance: cleanup_candidates returns list",
    isinstance(_mm_candidates, list),
    f"type={type(_mm_candidates).__name__}",
)

# Test 4: each cleanup candidate is a dict (if any)
_mm_cand_ok = all(isinstance(c, dict) for c in _mm_candidates[:5])
test(
    "MemoryMaintenance: cleanup candidates are dicts",
...
test(
    "RulesValidator: validate_rules returns dict",
    isinstance(_rv_report, dict),
    f"type={type(_rv_report).__name__}",
)

# Test 2: report has total and valid keys
test(
    "RulesValidator: report has total and valid keys",
...
test(
    "RulesValidator: total >= valid count",
...
test(
    "RulesValidator: report has issues field",
...
test(
    "SearchCache: constructor creates cache with 0 entries",
    len(_sc_cache) == 0,
    f"len={len(_sc_cache)}",
)
...
test(
    "SearchCache: put then get returns same value",
    _sc_got == {"result": "data"},
    f"got {_sc_got!r}",
)
...
test(
    "SearchCache: get on missing key returns None",
    _sc_miss is None,
    f"got {_sc_miss!r}",
)
...
test(
    "SearchCache: stats has hit_rate key",
...
test(
    "SearchCache: invalidate clears all entries",
    len(_sc_cache) == 0,
    f"len={len(_sc_cache)}",
)
...
test(
    "ConfigValidator: validate_all returns dict",
    isinstance(_cv_report, dict),
    f"type={type(_cv_report).__name__}",
)

# Test 2: report has settings key
test(
    "ConfigValidator: report has settings key",
    "settings" in _cv_report,
    f"keys={set(_cv_report.keys())}",
)

# Test 3: validate_settings returns list
_cv_settings = validate_settings()
test(
    "ConfigValidator: validate_settings returns list",
    isinstance(_cv_settings, list),
    f"type={type(_cv_settings).__name__}",
)

# Test 4: validate_gates returns list
_cv_gates = validate_gates()
test(
    "ConfigValidator: validate_gates returns list",
    isinstance(_cv_gates, list),
    f"type={type(_cv_gates).__name__}",
)
...
test(
    "GateRegistry: GATE_MODULES is non-empty list",
...
test(
    "GateRegistry: all modules start with 'gates.'",
    _gr_ok,
    f"sample={gr_GATE_MODULES[:3]}",
)

# Test 3: contains gate_01 and gate_02 (Tier 1 safety)
_gr_has_t1 = any("gate_01" in m for m in gr_GATE_MODULES) and any(
    "gate_02" in m for m in gr_GATE_MODULES
)
test(
    "GateRegistry: includes Tier 1 safety gates (01, 02)",
    _gr_has_t1,
    f"modules={gr_GATE_MODULES[:5]}",
)
...
test(
    "ToolFingerprint: fingerprint_tool returns hex string",
...
test("ToolFingerprint: deterministic fingerprinting", _tf_fp == _tf_fp2)

# Test 3: different input produces different fingerprint
_tf_fp3 = fingerprint_tool("__other_tool__", "Different tool", {})
test("ToolFingerprint: different tools get different fingerprints", _tf_fp != _tf_fp3)

# Test 4: register_tool returns tuple
_tf_reg = register_tool("__test_reg_tool__", "test", {})
test(
    "ToolFingerprint: register_tool returns 4-tuple",
...
test(
    "ToolFingerprint: get_all_fingerprints returns dict",
    isinstance(_tf_all, dict),
    f"type={type(_tf_all).__name__}",
)
...
try:
    ...
    test(
        "Hotspot: extract Edit file_path",
        _ch_edit == "/home/user/test.py",
        f"got '{_ch_edit}'",
    )
    ...
    test(
        "Hotspot: extract Write file_path",
        _ch_write == "/tmp/output.json",
        f"got '{_ch_write}'",
    )
    ...
    test(
        "Hotspot: extract Read path",
        _ch_read == "/home/user/readme.md",
        f"got '{_ch_read}'",
    )
    ...
    test(
        "Hotspot: analyze returns dict",
        isinstance(_ch_analysis, dict),
        f"type={type(_ch_analysis)}",
    )
    test(
        "Hotspot: analysis has file_blocks",
        "file_blocks" in _ch_analysis,
        f"keys={set(_ch_analysis.keys())}",
    )
    test(
        "Hotspot: analysis has total_blocks",
        "total_blocks" in _ch_analysis,
        f"keys={set(_ch_analysis.keys())}",
    )
    test(
        "Hotspot: file_blocks is list",
        isinstance(_ch_analysis["file_blocks"], list),
        f"type={type(_ch_analysis['file_blocks'])}",
    )
    test(
        "Hotspot: total_blocks is int",
    ...
    test(
        "Hotspot: rank returns list",
        isinstance(_ch_ranked, list),
        f"type={type(_ch_ranked)}",
    )
    ...
    if _ch_ranked:
        ...
        test(
            "Hotspot: ranked entry has rank",
            "rank" in _ch_first and _ch_first["rank"] == 1,
            f"keys={set(_ch_first.keys())}",
        )
        test(
            "Hotspot: ranked entry has risk_score",
            "risk_score" in _ch_first
            and isinstance(_ch_first["risk_score"], (int, float)),
            f"score={_ch_first.get('risk_score')}",
        )
        test(
            "Hotspot: ranked entry has risk_level",
            _ch_first.get("risk_level") in ("critical", "high", "medium", "low"),
            f"level={_ch_first.get('risk_level')}",
        )
        test(
            "Hotspot: ranked entry has churn_factor",
            "churn_factor" in _ch_first,
            f"keys={set(_ch_first.keys())}",
        )
        test(
            "Hotspot: ranked entry has error_density",
            "error_density" in _ch_first,
            f"keys={set(_ch_first.keys())}",
        )
    else:
    ...
    test(
        "Hotspot: export returns string",
        isinstance(_ch_report, str),
        f"type={type(_ch_report)}",
    )
    test(
        "Hotspot: report has header",
        "File Hotspot Report" in _ch_report,
        "missing header",
    )
    test(
        "Hotspot: report has separator lines",
        "=" * 10 in _ch_report,
        "missing separators",
    )
...
try:
    ...
    if _sr_timeline["events"]:
        ...
        test(
            "Replay: event has timestamp",
            "timestamp" in _sr_ev,
            f"keys={set(_sr_ev.keys())}",
        )
        test(
            "Replay: event has type",
            _sr_ev.get("type") in ("BLOCK", "PASS", "WARN", "EVENT"),
            f"type={_sr_ev.get('type')}",
        )
        test("Replay: event has gate", "gate" in _sr_ev, f"keys={set(_sr_ev.keys())}")
        test("Replay: event has tool", "tool" in _sr_ev, f"keys={set(_sr_ev.keys())}")
    else:
    ...
    test(
        "Replay: gate_filter returns subset",
    ...
    test(
        "Replay: export_text returns string",
        isinstance(_sr_text, str),
        f"type={type(_sr_text)}",
    )
    test(
        "Replay: text has Session Timeline header",
        "Session Timeline" in _sr_text,
        "missing header",
    )
    test("Replay: text has separator", "=" * 10 in _sr_text, "missing separator")

    # export_mermaid: returns mermaid markdown
    _sr_mermaid = export_mermaid(lookback_hours=1)
    test(
        "Replay: export_mermaid returns string",
        isinstance(_sr_mermaid, str),
        f"type={type(_sr_mermaid)}",
    )
    test(
        "Replay: mermaid starts with code fence",
        "```mermaid" in _sr_mermaid,
        "missing mermaid fence",
    )
    test(
        "Replay: mermaid has sequenceDiagram",
        "sequenceDiagram" in _sr_mermaid,
    ...
    test(
        "Replay: stats returns dict",
        isinstance(_sr_stats, dict),
        f"type={type(_sr_stats)}",
    )
    _sr_stat_keys = {
        "total_events",
        "duration_minutes",
    ...
    test(
        "Replay: detect_patterns returns dict",
        isinstance(_sr_patterns, dict),
        f"type={type(_sr_patterns)}",
    )
    test(
        "Replay: patterns has 'patterns' list",
        isinstance(_sr_patterns.get("patterns", []), list),
        f"type={type(_sr_patterns.get('patterns'))}",
    )
    test(
        "Replay: patterns has 'healthy' bool",
        isinstance(_sr_patterns.get("healthy"), bool),
        f"type={type(_sr_patterns.get('healthy'))}",
    )
    test(
        "Replay: patterns has 'summary' string",
    ...
    if _sr_patterns["patterns"]:
        ...
        test(
            "Replay: pattern has type",
            _sr_pat.get("type")
            in ("consecutive_blocks", "rapid_blocks", "gate_dominance"),
            f"type={_sr_pat.get('type')}",
        )
        test(
            "Replay: pattern has severity",
            _sr_pat.get("severity") in ("info", "warn", "error"),
            f"severity={_sr_pat.get('severity')}",
        )
        test(
            "Replay: pattern has description",
...
try:
    ...
    test(
        "GateTimingExt: flush_timings persists data to disk",
        _gte_s1 is not None
        and _gte_s1["count"] == 1
        and abs(_gte_s1["avg_ms"] - 15.0) < 0.01,
    ...
    test(
        "GateTimingExt: flush_timings no-op when not dirty",
        True,
        "No crash on clean flush",
    )
    ...
    test(
        "GateTimingExt: min_ms tracks minimum across records",
    ...
    test(
        "GateTimingExt: by_tool tracks per-tool counts",
    ...
    test(
        "GateTimingExt: by_tool tracks per-tool total_ms",
    ...
    test(
        "GateTimingExt: slow_count increments only for > threshold",
    ...
    test(
        "GateTimingExt: count tracks all records (210)",
    ...
    test(
        "GateTimingExt: avg_ms computed correctly (10+20+30)/3=20",
    ...
    test(
        "GateTimingExt: _percentile empty list returns 0.0",
    ...
    test(
        "GateTimingExt: _percentile single element returns it",
    ...
    test(
        "GateTimingExt: _percentile p50 of [10,20,30,40,50] = 30",
    ...
    test(
        "GateTimingExt: SLA warn status for 50 < avg < 200",
    ...
    test(
        "GateTimingExt: SLA unknown for non-existent gate",
    ...
    test(
        "GateTimingExt: empty timing report message",
    ...
    test(
        "GateTimingExt: get_slow_gates returns {} with no data",
    ...
    test(
        "GateTimingExt: get_sla_report returns {} with no data",
...
try:
    ...
    test(
        "GateTrend: no snapshots returns stable direction",
    ...
    _gtr_snaps2 = [
        {
            "timestamp": 1000,
            "gates": {"test_gate": {"avg_ms": 10.0, "p95_ms": 15.0, "count": 5}},
        }
    ]
    _gtr_t2 = compute_gate_trend("test_gate", _gtr_snaps2)
    test(
        "GateTrend: 1 snapshot returns stable with first/last equal",
        _gtr_t2["direction"] == "stable"
        and _gtr_t2["data_points"] == 1
        and _gtr_t2["first_avg_ms"] == 10.0
        and _gtr_t2["last_avg_ms"] == 10.0,
        f"got {_gtr_t2}",
    )
    ...
    _gtr_snaps3 = [
        {
            "timestamp": 1000,
            "gates": {"g1": {"avg_ms": 10.0, "p95_ms": 12.0, "count": 5}},
        },
        {
            "timestamp": 2000,
            "gates": {"g1": {"avg_ms": 15.0, "p95_ms": 18.0, "count": 10}},
        },
    ]
    _gtr_t3 = compute_gate_trend("g1", _gtr_snaps3)
    test(
        "GateTrend: detects rising trend (10->15ms, +50%)",
    ...
    _gtr_snaps4 = [
        {
            "timestamp": 1000,
            "gates": {"g2": {"avg_ms": 50.0, "p95_ms": 60.0, "count": 5}},
        },
        {
            "timestamp": 2000,
            "gates": {"g2": {"avg_ms": 30.0, "p95_ms": 35.0, "count": 10}},
        },
    ]
    _gtr_t4 = compute_gate_trend("g2", _gtr_snaps4)
    test(
        "GateTrend: detects falling trend (50->30ms, -40%)",
    ...
    _gtr_snaps5 = [
        {
            "timestamp": 1000,
            "gates": {"g3": {"avg_ms": 10.0, "p95_ms": 12.0, "count": 5}},
        },
        {
            "timestamp": 2000,
            "gates": {"g3": {"avg_ms": 11.0, "p95_ms": 13.0, "count": 10}},
        },
    ]
    _gtr_t5 = compute_gate_trend("g3", _gtr_snaps5)
    test(
        "GateTrend: stable when change < 20% (10->11ms)",
    ...
    test(
        "GateTrend: missing gate returns stable with 0 data_points",
    ...
    test(
        "GateTrend: get_trend_report has expected structure",
    ...
    test(
        "GateTrend: gate_slow classified as rising",
    ...
    test(
        "GateTrend: gate_fast classified as falling",
    ...
    test(
        "GateTrend: format_trend_report returns string with header",
    ...
    test(
        "GateTrend: format_trend_report shows RISING gates",
    ...
    test(
        "GateTrend: format_trend_report shows FALLING gates",
    ...
    test(
        "GateTrend: _load_snapshots returns list",
    ...
    test(
        "GateTrend: _load_snapshots returns [] for corrupt file",
    ...
    _gtr_test_data = [
        {
            "timestamp": 999,
            "gates": {"test": {"avg_ms": 1.0, "p95_ms": 1.5, "count": 1}},
        }
    ]
    _save_snapshots(_gtr_test_data)
    _gtr_rt = _load_snapshots()
    test(
        "GateTrend: save+load roundtrip preserves data",
    ...
    test(
        "GateTrend: empty snapshots -> 0 gates and empty rising/falling",
...
try:
    ...
    test(
        "SessionAnalyticsExt: gate_block_rates pass/warn/block counts",
    ...
    test(
        "SessionAnalyticsExt: gate_block_rates total counts all decisions",
    ...
    test(
        "SessionAnalyticsExt: gate_block_rates empty -> {}",
    ...
    test(
        "SessionAnalyticsExt: error_frequency categorizes gate1 pattern",
    ...
    test(
        "SessionAnalyticsExt: error_frequency categorizes gate2 pattern",
    ...
    test(
        "SessionAnalyticsExt: error_frequency categorizes gate4 memory pattern",
    ...
    test(
        "SessionAnalyticsExt: error_frequency skips pass decisions",
    ...
    test(
        "SessionAnalyticsExt: _stddev([10,20,30,40,50]) ≈ 14.14",
    ...
    test(
        "SessionAnalyticsExt: _stddev single value returns 0",
        _stddev([42.0]) == 0.0,
        f"got {_stddev([42.0])}",
    )
    ...
    test(
        "SessionAnalyticsExt: _stddev empty list returns 0",
        _stddev([]) == 0.0,
        f"got {_stddev([])}",
    )
    ...
    test(
        "SessionAnalyticsExt: _compute_resolve_score no blocks -> 1.0",
    ...
    test(
        "SessionAnalyticsExt: block then pass on same gate -> resolve=1.0",
    ...
    test(
        "SessionAnalyticsExt: unresolved block -> resolve=0.0",
    ...
    test(
        "SessionAnalyticsExt: productivity has score, grade, breakdown",
    ...
    test(
        "SessionAnalyticsExt: productivity score in range 0-100",
    ...
    test(
        "SessionAnalyticsExt: productivity grade is A-F",
    ...
    test(
        "SessionAnalyticsExt: breakdown has 4 sub-metrics",
        all(
            k in _sae_bd
            for k in ("edit_velocity", "block_rate", "error_resolve", "memory_contrib")
        ),
    ...
    test(
        "SessionAnalyticsExt: block_rate blocked_count=1",
    ...
    test(
        "SessionAnalyticsExt: memory_contrib counts 2 memory calls",
    ...
    test(
        "SessionAnalyticsExt: compare_sessions_metrics has expected keys",
    ...
    test(
        "SessionAnalyticsExt: rolling_avg = (70+75+65)/3 = 70.0",
    ...
    test(
        "SessionAnalyticsExt: positive delta -> improving trend",
    ...
    test(
        "SessionAnalyticsExt: empty history -> insufficient_data",
    ...
    test(
        "SessionAnalyticsExt: parse_audit_log parses valid lines, skips bad",
    ...
    test(
        "SessionAnalyticsExt: compare_sessions legacy mode works",
    ...
    test(
        "SessionAnalyticsExt: compare_sessions ID mode returns summary",
    ...
    test(
        "SessionAnalyticsExt: compare_sessions missing sessions in summary",
    ...
    test(
        "SessionAnalyticsExt: analyse_session with empty dir returns valid structure",
    ...
    test(
        "SessionAnalyticsExt: analyse_session empty dir -> entry_count=0",
    ...
    test(
        "SessionAnalyticsExt: productivity with 0 duration doesn't crash",
    ...
    test(
        "SessionAnalyticsExt: spike_detected when current far from history",
...
try:
    ...
    test(
        "HookCacheExt: get_cached_module('json') returns module",
    ...
    test(
        "HookCacheExt: repeated import is a cache hit",
    ...
    test(
        "HookCacheExt: invalidate_module returns True for cached module",
        _hce_inv_mod is True,
        f"got {_hce_inv_mod}",
    )
    ...
    test(
        "HookCacheExt: invalidate_module returns False for uncached",
        _hce_inv_mod2 is False,
        f"got {_hce_inv_mod2}",
    )
    ...
    _hce_s5 = cache_stats()
    test(
        "HookCacheExt: module_cached counts loaded modules",
    ...
    test(
        "HookCacheExt: state cache miss returns None",
        _hce_miss is None,
        f"got {_hce_miss}",
    )
    ...
    _hce_s7 = cache_stats()
    test(
        "HookCacheExt: state_misses incremented on miss",
    ...
    _hce_s8 = cache_stats()
    test(
        "HookCacheExt: state_hits incremented on hit",
    ...
    test(
        "HookCacheExt: state_cached = 1 after one set",
    ...
    test(
        "HookCacheExt: result cache set/get roundtrip",
    ...
    test(
        "HookCacheExt: result cache miss for different hash",
        _hce_r11 is None,
        f"got {_hce_r11}",
    )
    ...
    test(
        "HookCacheExt: invalidate_result removes entry",
    ...
    test(
        "HookCacheExt: invalidate_result False for missing",
        _hce_inv_r2 is False,
        f"got {_hce_inv_r2}",
    )
    ...
    test(
        "HookCacheExt: evict_expired returns {state: 0, result: 0} when empty",
    ...
    test(
        "HookCacheExt: clear_cache resets all counters to 0",
...
try:
    ...
    test(
        "MetricsCollectorExt: counter increments to 8",
    ...
    test(
        "MetricsCollectorExt: labeled counters are independent",
    ...
    test(
        "MetricsCollectorExt: gauge overwrites to 42.0",
    ...
    test(
        "MetricsCollectorExt: histogram count/sum/min/max",
        _mce_h4.get("count", 0) >= 3 and _mce_h4.get("type") == TYPE_HISTOGRAM,
        f"count={_mce_h4.get('count')}",
    )

    # Test 5: histogram has computed avg
    test(
        "MetricsCollectorExt: histogram has avg field",
    ...
    test(
        "MetricsCollectorExt: _label_key sorts keys",
        _mce_lk6 == "a=1,b=2",
        f"got '{_mce_lk6}'",
    )
    ...
    test(
        "MetricsCollectorExt: _label_key(None) returns ''",
    ...
    test(
        "MetricsCollectorExt: get_metric returns {} for missing",
        _mce_m8 == {},
        f"got {_mce_m8}",
    )
    ...
    test(
        "MetricsCollectorExt: get_all_metrics contains test counter",
    ...
    test(
        "MetricsCollectorExt: rollup(60) returns dict",
    ...
    test(
        "MetricsCollectorExt: rollup includes counter metrics",
        _mce_has_counter,
    ...
    test(
        "MetricsCollectorExt: record_gate_fire increments gate.fires",
    ...
    test(
        "MetricsCollectorExt: record_gate_block increments gate.blocks",
    ...
    test(
        "MetricsCollectorExt: record_gate_latency records histogram",
    ...
    test(
        "MetricsCollectorExt: set_test_pass_rate clamps 1.5 -> 1.0",
    ...
    test(
        "MetricsCollectorExt: export_json is valid JSON with expected keys",
    ...
    test(
        "MetricsCollectorExt: timed context manager records observation",
    ...
    _mce_f18 = mc_flush()
    test("MetricsCollectorExt: flush returns True", _mce_f18 is True, f"got {_mce_f18}")
...
try:
    ...
    test(
        "EventReplayExt: Read is always allowed",
        _is_always_allowed("Read") is True,
        "Expected True",
    )

    # Test 2: _is_always_allowed for Edit
    test(
        "EventReplayExt: Edit is NOT always allowed",
        _is_always_allowed("Edit") is False,
        "Expected False",
    )

    # Test 3: _is_memory_tool for mcp__memory__ prefix
    test(
        "EventReplayExt: mcp__memory__search_knowledge is memory tool",
        _is_memory_tool("mcp__memory__search_knowledge") is True,
        "Expected True",
    )

    # Test 4: _is_memory_tool for non-memory tool
    test(
        "EventReplayExt: Edit is not memory tool",
        _is_memory_tool("Edit") is False,
        "Expected False",
    )
    ...
    test(
        "EventReplayExt: _parse_context parses valid JSON",
    ...
    test(
        "EventReplayExt: _parse_context non-JSON returns {}",
        _ere_ctx6 == {},
        f"got {_ere_ctx6}",
    )
    ...
    test(
        "EventReplayExt: _parse_context('') returns {}",
        _parse_context("") == {},
        f"got {_parse_context('')}",
    )

    # Test 8: _extract_tool_input for Bash
    _ere_ti8 = _extract_tool_input({"tool_name": "Bash", "context": "ls -la"})
    test(
        "EventReplayExt: _extract_tool_input Bash has command",
        "command" in _ere_ti8,
        f"got {_ere_ti8}",
    )

    # Test 9: _extract_tool_input for Edit
    _ere_ti9 = _extract_tool_input(
        {"tool_name": "Edit", "context": '{"file_path": "/tmp/foo.py"}'}
    )
    test(
        "EventReplayExt: _extract_tool_input Edit has file_path",
        "file_path" in _ere_ti9,
        f"got {_ere_ti9}",
    )

    # Test 10: _extract_tool_input for Task
    _ere_ti10 = _extract_tool_input(
        {"tool_name": "Task", "context": '{"model": "haiku"}'}
    )
    test(
        "EventReplayExt: _extract_tool_input Task has model",
        "model" in _ere_ti10,
        f"got {_ere_ti10}",
    )
    ...
    test(
        "EventReplayExt: _build_replay_state returns state with session_id",
    ...
    test(
        "EventReplayExt: replay state has memory_last_queried",
    ...
    test(
        "EventReplayExt: load_events missing file returns []",
        isinstance(_ere_ev13, list) and len(_ere_ev13) == 0,
        f"len={len(_ere_ev13)}",
    )

    # Test 14: load_events on valid JSONL
    _ere_tmp14 = _ere_tempfile.NamedTemporaryFile(
        suffix=".jsonl", delete=False, mode="w"
    )
    _ere_tmp14.write(
        '{"document": "test", "metadata": {"tool_name": "Edit", "exit_code": "0", "session_id": "s1"}, "id": "obs_1"}\n'
    )
    _ere_tmp14.write(
        '{"document": "test2", "metadata": {"tool_name": "Bash", "exit_code": "2", "session_id": "s1", "gate": "gate_02"}, "id": "obs_2"}\n'
    )
    _ere_tmp14.close()
    _ere_ev14 = load_events(_ere_tmp14.name)
    test(
        "EventReplayExt: load_events parses 2 valid entries",
        len(_ere_ev14) == 2,
        f"len={len(_ere_ev14)}",
    )
    ...
    test(
        "EventReplayExt: filter_events tool_name='Edit' returns 1",
        len(_ere_f15) == 1
        and _ere_f15[0].get("_replay_meta", {}).get("tool_name") == "Edit",
        f"len={len(_ere_f15)}",
    )
    ...
    test(
        "EventReplayExt: filter_events blocked=True returns blocked events",
        len(_ere_f16) == 1
        and _ere_f16[0].get("_replay_meta", {}).get("originally_blocked") is True,
        f"len={len(_ere_f16)}",
    )
    ...
    test(
        "EventReplayExt: filter_events adds _replay_meta to all entries",
    ...
    test(
        "EventReplayExt: replay always-allowed tool -> skipped",
    ...
    test(
        "EventReplayExt: replay empty tool_name -> skipped",
    ...
    test(
        "EventReplayExt: diff_results no change -> changed=False",
    ...
    test(
        "EventReplayExt: diff_results passed->blocked -> changed=True",
    ...
    test(
        "EventReplayExt: diff_results per-gate detects gate_01 change",
    ...
    test(
        "EventReplayExt: summarise_replay empty -> total=0",
    ...
    test(
        "EventReplayExt: summarise_replay counts 1 changed, 1 unchanged",
        _ere_sum24["total"] == 2
        and _ere_sum24["changed"] == 1
        and _ere_sum24["unchanged"] == 1,
...
try:
    # --- extract_file_path ---
    test(
        "GateHelpers: extract_file_path from Edit input",
        gh_extract_file_path({"file_path": "/tmp/foo.py"}) == "/tmp/foo.py",
        f"got {gh_extract_file_path({'file_path': '/tmp/foo.py'})}",
    )

    test(
        "GateHelpers: extract_file_path from NotebookEdit input",
        gh_extract_file_path({"notebook_path": "/tmp/nb.ipynb"}) == "/tmp/nb.ipynb",
        f"got {gh_extract_file_path({'notebook_path': '/tmp/nb.ipynb'})}",
    )

    test(
        "GateHelpers: extract_file_path from Glob path",
        gh_extract_file_path({"path": "/tmp/dir"}) == "/tmp/dir",
        f"got {gh_extract_file_path({'path': '/tmp/dir'})}",
    )

    test(
        "GateHelpers: extract_file_path empty dict returns ''",
        gh_extract_file_path({}) == "",
        f"got '{gh_extract_file_path({})}'",
    )

    test(
        "GateHelpers: extract_file_path non-dict returns ''",
        gh_extract_file_path("not a dict") == "",
        "",
    )

    test(
        "GateHelpers: extract_file_path prefers file_path over path",
    ...
    # --- is_test_file ---
    test(
        "GateHelpers: is_test_file('test_foo.py') = True",
        gh_is_test_file("test_foo.py") is True,
        "",
    )

    test(
        "GateHelpers: is_test_file('foo_test.py') = True",
        gh_is_test_file("foo_test.py") is True,
        "",
    )

    test(
        "GateHelpers: is_test_file('foo_spec.py') = True",
        gh_is_test_file("foo_spec.py") is True,
        "",
    )

    test(
        "GateHelpers: is_test_file('foo.test.js') = True",
        gh_is_test_file("foo.test.js") is True,
        "",
    )

    test(
        "GateHelpers: is_test_file('foo.py') = False",
        gh_is_test_file("foo.py") is False,
        "",
    )

    test("GateHelpers: is_test_file('') = False", gh_is_test_file("") is False, "")

    # --- stem_normalize ---
    test(
        "GateHelpers: stem_normalize('test_foo.py') = 'foo'",
        gh_stem_normalize("test_foo.py") == "foo",
        f"got '{gh_stem_normalize('test_foo.py')}'",
    )

    test(
        "GateHelpers: stem_normalize('foo_test.py') = 'foo'",
        gh_stem_normalize("foo_test.py") == "foo",
        f"got '{gh_stem_normalize('foo_test.py')}'",
    )

    test(
        "GateHelpers: stem_normalize('foo.py') = 'foo'",
        gh_stem_normalize("foo.py") == "foo",
        f"got '{gh_stem_normalize('foo.py')}'",
    )

    test("GateHelpers: stem_normalize('') = ''", gh_stem_normalize("") == "", "")

    test(
        "GateHelpers: stem_normalize preserves non-test stems",
    ...
    # --- is_related_file ---
    test(
        "GateHelpers: foo.py related to test_foo.py",
        gh_is_related_file("foo.py", "test_foo.py") is True,
        "",
    )

    test(
        "GateHelpers: same basename in different dir is related",
        gh_is_related_file("/a/b/foo.py", "/c/d/foo.py") is True,
        "",
    )

    test(
        "GateHelpers: unrelated files are not related",
        gh_is_related_file("foo.py", "bar.py") is False,
        "",
    )

    test(
        "GateHelpers: empty paths are not related",
    ...
    # --- safe_tool_input ---
    test(
        "GateHelpers: safe_tool_input(dict) returns same dict",
        gh_safe_tool_input({"a": 1}) == {"a": 1},
        "",
    )

    test(
        "GateHelpers: safe_tool_input(None) returns {}",
        gh_safe_tool_input(None) == {},
        "",
    )

    test(
        "GateHelpers: safe_tool_input('str') returns {}",
    ...
    # --- extract_command ---
    test(
        "GateHelpers: extract_command from Bash input",
        gh_extract_command({"command": "ls -la"}) == "ls -la",
        "",
    )

    test(
        "GateHelpers: extract_command empty dict returns ''",
        gh_extract_command({}) == "",
        "",
    )

    test(
        "GateHelpers: extract_command non-dict returns ''",
    ...
    # --- is_edit_tool ---
    test(
        "GateHelpers: is_edit_tool('Edit') = True", gh_is_edit_tool("Edit") is True, ""
    )

    test(
        "GateHelpers: is_edit_tool('Write') = True",
        gh_is_edit_tool("Write") is True,
        "",
    )

    test(
        "GateHelpers: is_edit_tool('Bash') = False",
    ...
    # --- file_extension ---
    test(
        "GateHelpers: file_extension('foo.py') = '.py'",
        gh_file_extension("foo.py") == ".py",
        "",
    )

    test(
        "GateHelpers: file_extension('FOO.JSON') = '.json'",
        gh_file_extension("FOO.JSON") == ".json",
        "",
    )

    test(
        "GateHelpers: file_extension('noext') = ''",
        gh_file_extension("noext") == "",
        "",
    )

    test("GateHelpers: file_extension('') = ''", gh_file_extension("") == "", "")

    # --- elapsed_since ---
    _gh_recent = _gh_time.time() - 5.0
    _gh_elapsed = gh_elapsed_since(_gh_recent)
    test(
        "GateHelpers: elapsed_since 5s ago is ~5s",
        4.0 < _gh_elapsed < 10.0,
        f"got {_gh_elapsed}",
    )

    test("GateHelpers: elapsed_since(0) returns 0.0", gh_elapsed_since(0) == 0.0, "")

    test(
        "GateHelpers: elapsed_since(None) returns 0.0",
    ...
    # --- is_stale ---
    test(
        "GateHelpers: is_stale(recent, 60) = False",
        gh_is_stale(_gh_time.time() - 5, 60) is False,
        "",
    )

    test(
        "GateHelpers: is_stale(old, 10) = True",
        gh_is_stale(_gh_time.time() - 30, 10) is True,
        "",
    )

    test(
        "GateHelpers: is_stale(0, 60) = True (missing timestamp)",
...
try:
    ...
    test(
        "HotReload: discover_gate_modules returns list",
    ...
    test(
        "HotReload: all modules start with 'gates.'",
        _hr_all_prefixed,
        f"first non-prefixed: {[g for g in _hr_gates if not g.startswith('gates.')][:1]}",
    )

    # Test 3: _module_to_filepath converts dotted name to .py path
    _hr_path = _module_to_filepath("gates.gate_01_read_before_edit")
    test(
        "HotReload: _module_to_filepath resolves to .py file",
    ...
    test(
        "HotReload: _get_mtime returns float for existing file",
    ...
    test(
        "HotReload: _get_mtime returns None for missing file",
    ...
    test(
        "HotReload: _validate_module True for module with check()",
    ...
    test(
        "HotReload: _validate_module False for module without check()",
    ...
    test(
        "HotReload: seed_mtimes runs without error",
    ...
    test(
        "HotReload: check_for_changes {} after seeding",
    ...
    test(
        "HotReload: all modules changed when cache empty",
    ...
    if _hr_gates:
        ...
        test(
            "HotReload: reload_gate returns True for real gate",
    ...
    test(
        "HotReload: reload_gate False for missing gate",
        _hr_bad is False,
        f"got {_hr_bad}",
    )
    ...
    test(
        "HotReload: get_reload_history returns list",
        isinstance(_hr_hist, list),
        f"type={type(_hr_hist).__name__}",
    )
    ...
    if _hr_hist:
        ...
        _hr_has_keys = all(
            k in _hr_entry for k in ("module", "success", "timestamp", "reason")
        )
        test(
            "HotReload: history entry has expected keys",
            _hr_has_keys,
    ...
    test(
        "HotReload: reset_state clears history",
    ...
    test(
        "HotReload: auto_reload returns [] when nothing changed",
    ...
    test(
        "HotReload: auto_reload [] before interval elapses",
    ...
    test(
        "HotReload: discover_gate_modules custom dir finds gate_99_test",
    ...
    test(
        "HotReload: discover_gate_modules missing dir returns []",
        _hr_bad_dir == [],
        f"got {_hr_bad_dir}",
    )
...
try:
    ...
    _dc = detect_cycles()
    test(
        "DepGraph: detect_cycles returns has_cycles key",
        "has_cycles" in _dc,
        f"keys={list(_dc.keys())}",
    )

    # Test 2: detect_cycles returns cycles list
    test(
        "DepGraph: detect_cycles returns cycles list",
        isinstance(_dc.get("cycles"), list),
        f"type={type(_dc.get('cycles'))}",
    )

    # Test 3: detect_cycles returns summary string
    test(
        "DepGraph: detect_cycles returns summary string",
    ...
    test(
        "DepGraph: recommend_ordering returns ordering list",
        isinstance(_ro.get("ordering"), list),
        f"keys={list(_ro.keys())}",
    )

    # Test 5: recommend_gate_ordering returns tiers
    test(
        "DepGraph: recommend_ordering returns tiers list",
        isinstance(_ro.get("tiers"), list),
        f"type={type(_ro.get('tiers'))}",
    )

    # Test 6: recommend_gate_ordering has_cycles is bool
    test(
        "DepGraph: recommend_ordering has_cycles is bool",
        isinstance(_ro.get("has_cycles"), bool),
        f"type={type(_ro.get('has_cycles'))}",
    )

    # Test 7: format_dependency_report includes cycle section
    _fdr = format_dependency_report()
    test(
        "DepGraph: format_report includes Cycle Detection",
        "Cycle Detection" in _fdr,
        f"report snippet={_fdr[:100]}",
    )

    # Test 8: format_dependency_report includes ordering section
    test(
        "DepGraph: format_report includes Recommended Gate Ordering",
    ...
    test(
        "DepGraph: acyclic graph has no cycles",
        _dc_acyclic["has_cycles"] is False,
        f"cycles={_dc_acyclic['cycles']}",
    )

    # Test 10: recommend_ordering on acyclic graph gives valid topo order
    _ro_acyclic = recommend_gate_ordering()
    test(
        "DepGraph: acyclic ordering has all gates",
    ...
    test(
        "DepGraph: acyclic ordering respects deps (a < b < c)",
        _idx_a < _idx_b < _idx_c,
        f"indices a={_idx_a} b={_idx_b} c={_idx_c}",
    )

    # Test 12: tiers on acyclic graph have 3 tiers
    test(
        "DepGraph: acyclic graph has 3 tiers",
    ...
    test(
        "DepGraph: cyclic graph detects cycle",
        _dc_cyclic["has_cycles"] is True,
        f"cycles={_dc_cyclic['cycles']}",
    )
    ...
    test(
        "DepGraph: cyclic graph cycle contains gate_a and gate_b",
    ...
    test(
        "DepGraph: cyclic ordering includes all gates",
    ...
    test(
        "DepGraph: cyclic ordering reports has_cycles",
        _ro_cyclic["has_cycles"] is True,
        f"has_cycles={_ro_cyclic['has_cycles']}",
    )
    ...
    test(
        "DepGraph: empty deps has no cycles",
    ...
    test(
        "DepGraph: empty deps ordering is empty list",
    ...
    test(
        "DepGraph: independent gates have no cycles",
        _dc_indep["has_cycles"] is False,
        f"cycles={_dc_indep['cycles']}",
    )

    _ro_indep = recommend_gate_ordering()
    test(
        "DepGraph: independent gates all in tier 0",
    ...
    test(
        "DepGraph: find_state_conflicts returns list",
        isinstance(_conflicts, list),
        f"type={type(_conflicts)}",
    )

    # Test 22: find_parallel_safe_gates returns dict with expected keys
    _parallel = find_parallel_safe_gates()
    test(
        "DepGraph: parallel_safe_gates has expected keys",
        all(
            k in _parallel
            for k in ("independent_gates", "conflict_pairs", "total_gates")
        ),
    ...
    test(
        "DepGraph: get_state_hotspots returns list",
        isinstance(_hotspots, list),
        f"type={type(_hotspots)}",
    )

    # Test 24: generate_mermaid_diagram returns string with mermaid
    _mermaid = generate_mermaid_diagram()
    test(
        "DepGraph: mermaid diagram contains mermaid tag",
        "```mermaid" in _mermaid,
        f"snippet={_mermaid[:50]}",
    )
...
try:
    ...
    test(
        "MCP: crash_proof returns error dict on exception",
    ...
    _cp_ok = _ok_tool()
    test(
        "MCP: crash_proof passes through normal return",
        _cp_ok == {"status": "ok"},
        f"result={_cp_ok}",
    )
    ...
    test(
        "MCP: _detect_session_id returns string",
        isinstance(_sid, str) and len(_sid) > 0,
        f"sid={_sid}",
    )
    ...
    test(
        "MCP: _resolve_session_id explicit returns same",
        _rsid == "my-test-session",
        f"rsid={_rsid}",
    )
    ...
    test(
        "MCP: _resolve_session_id empty auto-detects",
    ...
    test(
        "MCP: framework_summary returns dict",
        isinstance(_fs, dict),
        f"type={type(_fs)}",
    )
    test(
        "MCP: framework_summary has health_score",
        "health_score" in _fs,
        f"keys={list(_fs.keys())}",
    )
    test(
        "MCP: framework_summary has skills_total",
        "skills_total" in _fs,
        f"keys={list(_fs.keys())}",
    )
    ...
    _cs = circuit_states()
    test("MCP: circuit_states returns dict", isinstance(_cs, dict), f"type={type(_cs)}")
    test(
        "MCP: circuit_states has services key",
        "services" in _cs,
        f"keys={list(_cs.keys())}",
    )
    test(
        "MCP: circuit_states has gates key", "gates" in _cs, f"keys={list(_cs.keys())}"
    )
    test(
        "MCP: circuit_states total_services is int",
        isinstance(_cs.get("total_services"), int),
        f"val={_cs.get('total_services')}",
    )
    ...
    test(
        "MCP: gate_dependencies returns dict",
        isinstance(_gd, dict),
        f"type={type(_gd)}",
    )
    test(
        "MCP: gate_dependencies has conflicts key",
        "conflicts" in _gd,
        f"keys={list(_gd.keys())}",
    )
    test(
        "MCP: gate_dependencies has cycles key",
        "cycles" in _gd,
        f"keys={list(_gd.keys())}",
    )
    test(
        "MCP: gate_dependencies has recommended_ordering key",
        "recommended_ordering" in _gd,
        f"keys={list(_gd.keys())}",
    )
    test(
        "MCP: gate_dependencies cycles has has_cycles",
        "has_cycles" in _gd.get("cycles", {}),
        f"cycles_keys={list(_gd.get('cycles', {}).keys())}",
    )
    test(
        "MCP: gate_dependencies ordering has ordering list",
    ...
    _ch = cache_health()
    test("MCP: cache_health returns dict", isinstance(_ch, dict), f"type={type(_ch)}")
    test(
        "MCP: cache_health has module_cache",
        "module_cache" in _ch,
        f"keys={list(_ch.keys())}",
    )
    test(
        "MCP: cache_health has state_cache",
        "state_cache" in _ch,
        f"keys={list(_ch.keys())}",
    )
    test(
        "MCP: cache_health has result_cache",
        "result_cache" in _ch,
        f"keys={list(_ch.keys())}",
    )
    test(
        "MCP: cache_health module hit_rate is float",
    ...
    test(
        "MCP: gate_sla_status returns dict",
        isinstance(_gss, dict),
        f"type={type(_gss)}",
    )
    test(
        "MCP: gate_sla_status has total_gates",
        "total_gates" in _gss,
        f"keys={list(_gss.keys())}",
    )
    test("MCP: gate_sla_status has ok count", "ok" in _gss, f"keys={list(_gss.keys())}")

    # Test gate_sla_status with custom threshold
    _gss_custom = gate_sla_status(threshold_ms=10)
    test(
        "MCP: gate_sla_status custom threshold returns dict",
    ...
    _es = event_stats()
    test("MCP: event_stats returns dict", isinstance(_es, dict), f"type={type(_es)}")
    test("MCP: event_stats has stats key", "stats" in _es, f"keys={list(_es.keys())}")
    test(
        "MCP: event_stats has recent_events",
        "recent_events" in _es,
        f"keys={list(_es.keys())}",
    )

    # Test event_stats with filter
    _es_filtered = event_stats(event_type="GATE_BLOCKED", limit=5)
    test(
        "MCP: event_stats filtered returns dict",
        isinstance(_es_filtered, dict),
        f"type={type(_es_filtered)}",
    )
    test(
        "MCP: event_stats filtered has filter field",
    ...
    _gt = gate_trends()
    test("MCP: gate_trends returns dict", isinstance(_gt, dict), f"type={type(_gt)}")
    test(
        "MCP: gate_trends has snapshot_count",
        "snapshot_count" in _gt,
        f"keys={list(_gt.keys())}",
    )
    ...
    _am = all_metrics()
    test("MCP: all_metrics returns dict", isinstance(_am, dict), f"type={type(_am)}")
    test(
        "MCP: all_metrics has current key", "current" in _am, f"keys={list(_am.keys())}"
    )
    test(
        "MCP: all_metrics has rollup_1m", "rollup_1m" in _am, f"keys={list(_am.keys())}"
    )
    test(
        "MCP: all_metrics has rollup_5m", "rollup_5m" in _am, f"keys={list(_am.keys())}"
    )
    ...
    test(
        "MCP: preview_gates Read returns dict",
        isinstance(_pg, dict),
        f"type={type(_pg)}",
    )
    test(
        "MCP: preview_gates has tool_name",
        _pg.get("tool_name") == "Read",
        f"tool_name={_pg.get('tool_name')}",
    )
    test(
        "MCP: preview_gates has would_block",
        "would_block" in _pg,
        f"keys={list(_pg.keys())}",
    )
    test(
        "MCP: preview_gates has gates_checked",
        isinstance(_pg.get("gates_checked"), int),
        f"val={_pg.get('gates_checked')}",
    )

    # Test preview_gates with Edit tool (more gates apply)
    _pg_edit = preview_gates("Edit", '{"file_path": "/tmp/test.py"}')
    test(
        "MCP: preview_gates Edit checks more gates",
    ...
    test(
        "MCP: preview_gates handles invalid JSON gracefully",
    ...
    _sh = skill_health()
    test("MCP: skill_health returns dict", isinstance(_sh, dict), f"type={type(_sh)}")
    test(
        "MCP: skill_health has total_skills key",
        "total_skills" in _sh,
        f"keys={list(_sh.keys())}",
    )
    ...
    _gdb = gate_dashboard()
    test(
        "MCP: gate_dashboard returns dict", isinstance(_gdb, dict), f"type={type(_gdb)}"
    )
    test(
        "MCP: gate_dashboard has dashboard text",
        "dashboard" in _gdb,
        f"keys={list(_gdb.keys())}",
    )
    test(
        "MCP: gate_dashboard has ranked_gates",
        "ranked_gates" in _gdb,
        f"keys={list(_gdb.keys())}",
    )
    ...
    _mh = memory_health()
    test("MCP: memory_health returns dict", isinstance(_mh, dict), f"type={type(_mh)}")
    test(
        "MCP: memory_health has surreal_exists",
        "surreal_exists" in _mh,
        f"keys={list(_mh.keys())}",
    )
    test(
        "MCP: memory_health has tables key", "tables" in _mh, f"keys={list(_mh.keys())}"
    )
    ...
    test("MCP: session_replay returns dict", isinstance(_sr, dict), f"type={type(_sr)}")
    test(
        "MCP: session_replay has event_count",
        "event_count" in _sr,
        f"keys={list(_sr.keys())}",
    )

    # Test session_replay mermaid format
    _sr_m = session_replay(format="mermaid")
    test(
        "MCP: session_replay mermaid returns dict with mermaid key",
        "mermaid" in _sr_m,
        f"keys={list(_sr_m.keys())}",
    )

    # Test session_replay stats format
    _sr_s = session_replay(format="stats")
    test(
        "MCP: session_replay stats returns dict",
    ...
    test(
        "MCP: session_replay patterns returns dict with healthy key",
        "healthy" in _sr_p,
        f"keys={list(_sr_p.keys())}",
    )
    ...
    test(
        "MCP: framework_pulse returns dict", isinstance(_fp, dict), f"type={type(_fp)}"
    )
    test(
        "MCP: framework_pulse has health_score",
        "health_score" in _fp,
        f"keys={list(_fp.keys())}",
    )
    test(
        "MCP: framework_pulse health_score 0-100",
        0 <= _fp.get("health_score", -1) <= 100,
        f"score={_fp.get('health_score')}",
    )
    test(
        "MCP: framework_pulse has hotspots",
        "hotspots" in _fp,
        f"keys={list(_fp.keys())}",
    )
    test(
        "MCP: framework_pulse has gate_trends",
        "gate_trends" in _fp,
        f"keys={list(_fp.keys())}",
    )
    test(
        "MCP: framework_pulse has circuits",
        "circuits" in _fp,
        f"keys={list(_fp.keys())}",
    )
    ...
    test(
        "MCP Analytics Integration: import and tests",
        False,
        f"{_mcp_exc}\n{_mcp_tb.format_exc()}",
    )
...
try:
    ...
    test(
        "MCP fix_effectiveness: returns dict",
        isinstance(_fe, dict),
        f"type={type(_fe)}",
    )
    test(
        "MCP fix_effectiveness: has total_fix_attempts",
        "total_fix_attempts" in _fe,
        f"keys={list(_fe.keys())}",
    )
    test(
        "MCP fix_effectiveness: has unique_errors",
        "unique_errors" in _fe,
        f"keys={list(_fe.keys())}",
    )
    test(
        "MCP fix_effectiveness: has overall_success_rate",
        "overall_success_rate" in _fe,
        f"keys={list(_fe.keys())}",
    )
    test(
        "MCP fix_effectiveness: overall_success_rate is float",
    ...
    test(
        "MCP fix_effectiveness: unknown error has empty strategies",
    ...
    test(
        "MCP query_observations: returns dict",
        isinstance(_qo, dict),
        f"type={type(_qo)}",
    )
    test(
        "MCP query_observations: has total key",
        "total" in _qo,
        f"keys={list(_qo.keys())}",
    )
    test(
        "MCP query_observations: has observations list",
    ...
    test(
        "MCP query_observations: error_only returns dict",
        isinstance(_qo_err, dict) and "total" in _qo_err,
        f"keys={list(_qo_err.keys())}",
    )
    test(
        "MCP query_observations: filters_applied correct",
    ...
    test(
        "MCP query_observations: tool_name filter applied",
    ...
    test(
        "MCP query_observations: priority filter applied",
    ...
    test(
        "MCP query_observations: has sentiment_breakdown",
    ...
    _id = inspect_domain()
    test("MCP inspect_domain: returns dict", isinstance(_id, dict), f"type={type(_id)}")
    test(
        "MCP inspect_domain: has active_domain key",
        "active_domain" in _id,
        f"keys={list(_id.keys())}",
    )
    test(
        "MCP inspect_domain: has total_domains",
        "total_domains" in _id,
        f"keys={list(_id.keys())}",
    )
    test(
        "MCP inspect_domain: has domains list",
        isinstance(_id.get("domains"), list),
        f"type={type(_id.get('domains'))}",
    )
    test(
        "MCP inspect_domain: total_domains matches list length",
    ...
    if _id.get("active_domain"):
        ...
        test(
            "MCP inspect_domain: active_detail has mastery",
            "mastery" in _detail,
            f"detail_keys={list(_detail.keys())}",
        )
        test(
            "MCP inspect_domain: active_detail has behavior",
            "behavior" in _detail,
            f"detail_keys={list(_detail.keys())}",
        )
        test(
            "MCP inspect_domain: active_detail has gate_overrides",
            "gate_overrides" in _detail,
            f"detail_keys={list(_detail.keys())}",
        )
        test(
            "MCP inspect_domain: active_detail has token_budget",
            "token_budget" in _detail,
            f"detail_keys={list(_detail.keys())}",
        )
        test(
            "MCP inspect_domain: active_detail has over_budget bool",
            isinstance(_detail.get("over_budget"), bool),
            f"type={type(_detail.get('over_budget'))}",
        )
        test(
            "MCP inspect_domain: active_detail has graduation",
            "graduation" in _detail,
            f"detail_keys={list(_detail.keys())}",
        )
    else:
        test(
            "MCP inspect_domain: no active domain → no active_detail",
            "active_detail" not in _id,
            f"keys={list(_id.keys())}",
        )
    ...
    test(
        "New MCP Tools: import and tests",
        False,
