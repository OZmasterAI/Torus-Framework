...
_NET_HOST = os.environ.get("SEARCH_HOST", "127.0.0.1")
_NET_PORT = int(os.environ.get("SEARCH_PORT", "8744"))
...
if _args.http:
    mcp = FastMCP("search", host=_NET_HOST, port=_args.port)
else:
    mcp = FastMCP("search")
...
if __name__ == "__main__":
    ...
    if _args.http:
        print(
            f"[Search MCP] Starting {_mode} on {_NET_HOST}:{_args.port}",
            file=sys.stderr,
        )
